...
export type ManagerFollowThroughItem = { recommendationId: string; title: string; workstream: string; priority: string; outcome: string; outcomeReason: string | null; actionType: string | null; canMutate: boolean; canAccept: boolean; canReconcile: boolean; state: ManagerFollowThroughState; stage: string; status: string; statusLabel: string; detail: string; nextAction: string; destination: { href: string; label: string } | null; target: { kind: string; id: string; label: string; title: string; status: string }; outcomeAt: string | null; updatedAt: string; dates: { createdAt: string; updatedAt: string; outcomeAt: string | null; dueAt: string | null; targetUpdatedAt: string | null } };
export type ManagerFollowThrough = { policyVersion: "manager_follow_through_v1"; observedAt: string; counts: { total: number; needsAction: number; inMotion: number; blocked: number; completed: number }; items: ManagerFollowThroughItem[] };
export type ManagerRecommendation = { id: string; title: string; reason: string; nextAction: string; priority: string; evidence: string[]; outcome: string; outcomeReason?: string | null; outcomeNote?: string | null; projectId?: string | null; eventId?: string | null; followThrough?: ManagerFollowThroughItem | null; proposedAction?: { type: "create_task"; title: string; dueAt?: string | null; initiativeId?: string | null } | { type: "create_conversation_task"; sourceMessageId: string; sourceMessageCreatedAt: string; title: string; dueDate: string | null; dateBasisTimezone: string | null } | { type: "update_conversation_task"; sourceMessageId: string; sourceMessageCreatedAt: string; taskId: string; taskUpdatedAt: string; taskTitle: string; operation: "complete" | "start" | "resume" | "block" | "reschedule" | "clear_due_date" | "set_waiting_on" | "clear_waiting_on"; dueDate: string | null; dateBasisTimezone: string | null; blockedReason: string | null; waitingOn: string | null } | { type: "assign_conversation_task"; sourceMessageId: string; sourceMessageCreatedAt: string; taskId: string; taskUpdatedAt: string; taskTitle: string; bandMemberId: string; bandMemberName: string; previousBandMemberId: string | null; previousOwnerLabel: string | null; checkInId: string | null; availability: "available" | "limited" | "unknown" } | { type: "create_conversation_project"; sourceMessageId: string; sourceMessageCreatedAt: string; projectType: "release" | "content_campaign" | "tour" | "business"; name: string; dueDate: string; planVersion: "project_plan_v1" } | { type: "create_conversation_event"; sourceMessageId: string; sourceMessageCreatedAt: string; eventType: "gig" | "rehearsal" | "studio" | "release" | "promotion" | "travel" | "meeting"; status: "draft" | "hold" | "confirmed"; title: string; startsAt: string; timezone: string; locationName: string | null; bandMemberIds: string[] } | { type: "update_conversation_event_availability"; sourceMessageId: string; sourceMessageCreatedAt: string; eventId: string; eventUpdatedAt: string; eventTitle: string; eventStartsAt: string | null; bandMemberId: string; bandMemberName: string; participantId: string | null; previousResponse: "unknown" | "available" | "tentative" | "unavailable"; previousRespondedAt: string | null; response: "unknown" | "available" | "tentative" | "unavailable" } | { type: "create_decision"; title: string; workstream: string; context?: string | null; options: ManagerDecisionOption[] } | { type: "generate_event_advance"; eventId: string } | { type: "prepare_event_logistics_approvals"; policyVersion: "event_logistics_v1"; eventId: string; eventFingerprint: string; channels: EventLogisticsChannel[]; retryChannels: EventLogisticsChannel[] } | { type: "generate_project_plan"; projectId: string } | { type: "remember_fact"; sourceMessageId: string; sourceMessageCreatedAt: string; key: string; label: string; value: string } | { type: "assign_task"; taskId: string; bandMemberId: string; checkInId: string | null; availability: "available" | "limited" | "unknown" } | { type: "update_profile_context"; profileId: string; profileUpdatedAt: string; gapCode: string; field: string; value: unknown } | null };
...
export type ManagerMessageAction = { recommendationId: string; title: string; nextAction: string; outcome: string; actionType?: "create_task" | "create_conversation_task" | "update_conversation_task" | "assign_conversation_task" | "create_conversation_project" | "create_conversation_event" | "update_conversation_event_availability" | "create_decision" | "generate_event_advance" | "prepare_event_logistics_approvals" | "generate_project_plan" | "remember_fact" | "assign_task" | "update_profile_context" | null; canAccept?: boolean; preview?: string; followThrough?: ManagerFollowThroughItem | null };
...
export type ManagerMessage = { id: string; role: "user" | "assistant"; content: string; citations: string[]; proposedActions: ManagerMessageAction[]; canSubmitFeedback: boolean; feedback?: ManagerMessageFeedback | null; createdAt: string };
...
export type EventLogisticsAssessment = {
  ...
  eligible: boolean;
  blockingReasons: ("status_not_confirmed" | "start_missing" | "end_missing" | "timezone_missing" | "timezone_invalid" | "invalid_time_range")[];
  channels: Record<EventLogisticsChannel, { channel: EventLogisticsChannel; state: EventLogisticsChannelState; approvalId: string | null; approvalStatus: string | null; attempt: number | null }>;
...
export type Song = { id: string; title: string; durationSeconds?: number | null; musicalKey?: string | null; bpm?: number | null; leadVocalist?: string | null; genre?: string | null; notes?: string | null; lyricsUrl?: string | null; chartUrl?: string | null; active: boolean; updatedAt?: string };
...
export type Setlist = { id: string; name: string; status: string; notes?: string | null; updatedAt?: string; summary?: SetlistSummary; items: { id: string; songId?: string | null; itemType: "song" | "break" | "note"; label?: string | null; transitionNotes?: string | null; song?: Song | null }[] };
