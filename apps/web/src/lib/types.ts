...
export type ManagerGoalPathItem = { goalId: string; goalTitle: string; workstream: string; deadline: string | null; progressRatio: number | null; status: "ready" | "in_progress" | "waiting" | "blocked" | "conflicted" | "missing_initiative" | "missing_task" | "needs_measurement" | "target_reached"; initiativeIds: string[]; nextTask: { taskId: string; title: string; state: ManagerWorkSequenceItem["state"]; pathType: "linked_task" | "prerequisite"; dueAt: string | null; ownerLabel: string | null; reason: string; evidenceIds: string[] } | null; contradictions: { code: "goal_deadline_passed" | "initiative_after_goal" | "task_after_goal" | "task_after_initiative" | "sequence_conflict"; detail: string; evidenceIds: string[] }[]; reason: string; nextAction: string; evidenceIds: string[] };
export type ManagerGoalPath = { policyVersion: "manager_goal_path_v1"; observedAt: string; status: "clear" | "needs_attention" | "blocked" | "conflicted" | "empty"; summary: string; counts: { activeGoals: number; ready: number; waiting: number; blocked: number; missingPlan: number; needsMeasurement: number; targetReached: number; conflicted: number }; goals: ManagerGoalPathItem[]; evidenceIds: string[] };
...
export type ManagerGoal = { id: string; sourceKey?: string | null; workstream: string; title: string; targetValue?: number | null; targetUnit?: string | null; currentValue?: number | null; measurementKind: ManagerGoalMeasurementKind; deadline?: string | null; status: string; initiatives?: ManagerInitiative[]; progressEvents?: ManagerGoalProgressEvent[] };
...
export type ManagerMessageAction = { recommendationId: string; title: string; nextAction: string; outcome: string; actionType?: "create_task" | "create_decision" | "generate_event_advance" | "generate_project_plan" | "remember_fact" | "assign_task" | null; preview?: string };
