...
export function CommandBar({ artistId }: { artistId?: string }) {
  ...
  return (
    <>
      <button
        type="button"
        className={`${mobileOpen ? "hidden" : "flex"} min-h-11 w-full items-center justify-between rounded-[var(--radius-lg)] border border-cyan-500/20 bg-[var(--surface-1)] px-4 text-left shadow-[0_0_30px_-14px_rgba(34,211,238,0.35)] md:hidden`}
        aria-expanded={mobileOpen}
        aria-controls="storyboard-command-panel"
      ...
      <section
        id="storyboard-command-panel"
        className={`${mobileOpen ? "block" : "hidden"} rounded-[var(--radius-xl)] border border-cyan-500/20 bg-[var(--surface-1)]/95 p-4 shadow-[0_0_40px_-12px_rgba(34,211,238,0.35)] md:block md:p-5`}
      ...
      >
        <div className="mb-3 flex items-center gap-2 text-[var(--text-primary)]">
          ...
          <button
            type="button"
            className="ml-auto inline-flex min-h-8 min-w-8 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--surface-2)] hover:text-[var(--text-primary)] md:hidden"
