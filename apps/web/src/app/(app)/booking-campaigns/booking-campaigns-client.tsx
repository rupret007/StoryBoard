...
export function BookingCampaignsClient({
  ...
  return (
    <div className="space-y-6">
      ...
      {previews.length > 0 ? <SurfaceCard><div className="flex items-center gap-2"><MailCheck className="h-4 w-4 text-[var(--accent)]" /><h2 className="text-sm font-semibold text-[var(--text-primary)]">Personalized email preview</h2></div><p className="mt-1 text-xs text-[var(--text-muted)]">{previewDeliveryMode === "send_on_execution" ? "These messages are pending approval. They send only after a person approves the batch and separately chooses Execute." : "These messages are pending approval. Execution creates Gmail drafts only; it never sends them."}</p><div className="mt-4 space-y-3">{previews.map((preview) => <div key={preview.recipientId} className="rounded-xl border border-[var(--border)] bg-[var(--surface-0)] p-3"><p className="text-xs text-[var(--text-muted)]">To: {preview.to}</p><p className="mt-1 font-medium text-[var(--text-primary)]">{preview.subject}</p><pre className="mt-2 whitespace-pre-wrap font-sans text-sm text-[var(--text-secondary)]">{preview.body}</pre><p className="mt-2 text-xs text-[var(--text-muted)]">Follow up: {(() => { const due = describeTaskDueDate(preview.followUpDueAt); return due ? `${due.label}${due.timing === "past" ? " (overdue)" : due.timing === "today" ? " (today)" : ""}` : "date unavailable"; })()}</p></div>)}</div><a className="sb-btn-secondary mt-4" href="/approvals"><CheckCircle2 className="h-4 w-4" />Open approval center</a></SurfaceCard> : null}
