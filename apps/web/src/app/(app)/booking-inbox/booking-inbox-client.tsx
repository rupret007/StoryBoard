...
export function BookingInboxClient({
  ...
  async function reload() {
    ...
    setSettings(await apiFetch<BookingReplySettings>("/booking-replies/settings"));
  }
  ...
  async function sync() {
    ...
    setNotice(null);
    try {
      const result = await apiFetch<{ created: number }>("/booking-replies/sync", {
        method: "POST"
      });
      await reload();
      setNotice(`${result.created} new repl${result.created === 1 ? "y" : "ies"} found.`);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Reply sync failed");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function action(id: string, path: string, json?: unknown) {
    ...
    setNotice(null);
    try {
      await apiFetch(`/booking-replies/${id}/${path}`, {
        method: "POST",
        ...(json ? { json } : {})
      });
      await reload();
  ...
  return (
    <div className="space-y-5">
      {notice ? (
        <div
          role="status"
          className="rounded-xl border border-[var(--border)] bg-[var(--surface-1)] px-4 py-3 text-sm text-[var(--text-secondary)]"
        >
          {notice}
        </div>
      ) : null}
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="font-semibold text-[var(--text-primary)]">
              Tracked Gmail threads
            </h2>
            <p className="mt-1 max-w-2xl text-sm text-[var(--text-muted)]">
              StoryBoard checks only campaign threads it created. It does not search or
              display your general inbox.
            </p>
          </div>
          <button
            type="button"
...
function ReplyCard({
  ...
  const [subject, setSubject] = useState(
    reply.suggestedReplySubject ??
      (reply.subject?.startsWith("Re:") ? reply.subject : `Re: ${reply.subject ?? reply.recipient.prospect.name}`)
  );
  const [body, setBody] = useState(reply.suggestedReplyBody ?? "");
  ...
  return (
    <SurfaceCard>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="font-semibold text-[var(--text-primary)]">
              {reply.recipient.prospect.name}
            </h2>
            <Badge variant={reply.processingStatus === "unread" ? "warning" : "neutral"}>
              {reply.intent ?? reply.processingStatus}
            </Badge>
          </div>
          <p className="mt-1 text-xs text-[var(--text-muted)]">
            {reply.fromName ?? reply.fromEmail} · {new Date(reply.receivedAt).toLocaleString()} · {reply.recipient.campaign.name}
          </p>
        </div>
        {aiEnabled && !reply.analyzedAt ? (
          <button
            type="button"
      ...
      <p className="mt-4 rounded-lg bg-[var(--surface-0)] p-3 text-sm text-[var(--text-secondary)]">
        {reply.snippet ?? "Open the tracked thread in Gmail to read the complete message."}
      </p>

      {reply.summary ? (
        <div className="mt-4 rounded-lg border border-[var(--border)] p-3">
          <p className="text-sm font-medium">AI summary</p>
          <p className="mt-1 text-sm text-[var(--text-secondary)]">{reply.summary}</p>
      ...
      <div className="mt-4 grid gap-2">
        ...
        <textarea
          ...
          placeholder="Write or paste the response you want Gmail to hold as a draft…"
        />
        <button
          type="button"
