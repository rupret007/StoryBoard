...
type Mutate = (path: string, json: unknown, method?: string) => Promise<unknown>;
...
export function OperationsClient({ initialTab = "events", focusEventId = null, focusField = null, eventsAvailable, readinessAvailable, setlistsAvailable, bookingsAvailable, initialEvents, initialReadiness, initialSongs, initialSetlists, initialBookings, initialProjects, initialDeals, initialInvoices, initialExpenses, initialSettlements, initialTemplates, members, contacts, venues, accessState, isOwner, loadError }: { initialTab?: Tab; focusEventId?: string | null; focusField?: OpsWorkspaceFocus | null; eventsAvailable: boolean; readinessAvailable: boolean; setlistsAvailable: boolean; bookingsAvailable: boolean; initialEvents: BandEvent[]; initialReadiness: ShowReadiness[]; initialSongs: Song[]; initialSetlists: Setlist[]; initialBookings: BookingOpportunity[]; initialProjects: ArtistProject[]; initialDeals: DealOffer[]; initialInvoices: Invoice[]; initialExpenses: Expense[]; initialSettlements: Settlement[]; initialTemplates: DocumentTemplate[]; members: BandMember[]; contacts: Contact[]; venues: Venue[]; accessState: OperationsAccessState; isOwner: boolean; loadError: string | null }) {
  ...
  async function create(path: string, json: unknown, method = "POST") {
    if (!canManage) {
    ...
    try {
      const result = await apiFetch(path, { method, json });
      router.refresh();
      return result;
    } catch (err) {
...
function EventAfterShowEditor({ event, expectedUpdatedAt, onReceipt, busy, create }: { event: BandEvent; expectedUpdatedAt?: string; onReceipt: (value: string) => void; busy: boolean; create: Mutate }) {
...
function Music({ songs, setlists, events, readiness, busy, create, openWorkspace }: { songs: Song[]; setlists: Setlist[]; events: BandEvent[]; readiness: ShowReadiness[]; busy: boolean; create: (path: string, json: unknown, method?: string) => Promise<void>; openWorkspace: OpenWorkspace }) {
  const [title, setTitle] = useState("");
...
type SetlistDraftItem = { songId: string | null; itemType: "song" | "break" | "note"; label: string; transitionNotes: string };

function SetlistBuilder({ setlist, songs, attachedEvents, busy, create, openWorkspace }: { setlist: Setlist; songs: Song[]; attachedEvents: BandEvent[]; busy: boolean; create: (path: string, json: unknown, method?: string) => Promise<void>; openWorkspace: OpenWorkspace }) {
  const [name, setName] = useState(setlist.name);
  const [status, setStatus] = useState(setlist.status);
  const [notes, setNotes] = useState(setlist.notes ?? "");
  const [songToAdd, setSongToAdd] = useState(songs.find((song) => song.active)?.id ?? "");
  const [items, setItems] = useState<SetlistDraftItem[]>(setlist.items.map((item) => ({ songId: item.songId ?? item.song?.id ?? null, itemType: item.itemType, label: item.label ?? "", transitionNotes: item.transitionNotes ?? "" })));
  const summary = summarizeSetlist(items.map((item, index) => ({ id: `${index}`, itemType: item.itemType, label: item.label, song: item.songId ? songs.find((song) => song.id === item.songId) ?? null : null })));
  const valid = Boolean(name.trim()) && items.every((item) => item.itemType === "song" ? Boolean(item.songId || item.label.trim()) : Boolean(item.label.trim()));
  const updateItem = (index: number, patch: Partial<SetlistDraftItem>) => setItems((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, ...patch } : item));
  ...
  const addSong = () => { if (!songToAdd || items.length >= 100) return; setItems((current) => [...current, { songId: songToAdd, itemType: "song", label: "", transitionNotes: "" }]); };
  const addMarker = (itemType: "break" | "note") => { if (items.length >= 100) return; setItems((current) => [...current, { songId: null, itemType, label: itemType === "break" ? "Set break" : "Talk / announcement", transitionNotes: "" }]); };
  return <details className="rounded-xl border border-[var(--border)] p-4" data-testid={`setlist-${setlist.id}`}><summary className="cursor-pointer list-none"><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{setlist.name}</p><p className="mt-1 text-xs text-[var(--text-muted)]">{summary.durationLabel} · {items.length} item{items.length === 1 ? "" : "s"} · {catalogSourceLabel(setlist.sourceKey)}</p>{attachedEvents.length ? <p className="mt-1 text-xs text-[var(--text-secondary)]">Attached to {attachedEvents.map((event) => event.title).join(", ")}</p> : <p className="mt-1 text-xs text-[var(--text-muted)]">Not attached to a show yet. Attach it from Events.</p>}{summary.timingStatus === "incomplete" ? <p className="mt-1 text-xs font-medium text-[var(--text-primary)]">Next: record every song duration in the library before relying on this set length.</p> : null}</div><Badge variant={setlist.status === "active" ? "success" : "neutral"}>{setlist.status}</Badge></div></summary><div className="mt-5 border-t border-[var(--border)] pt-4"><div className="grid gap-3 sm:grid-cols-2"><label><span className="sb-label">Setlist name</span><input aria-label={`Name for setlist ${setlist.name}`} className="sb-input mt-1.5" value={name} maxLength={240} onChange={(event) => setName(event.target.value)} /></label><label><span className="sb-label">Status</span><select aria-label={`Status for setlist ${setlist.name}`} className="sb-select mt-1.5" value={status} onChange={(event) => setStatus(event.target.value)}><option value="draft">draft</option><option value="active">active</option><option value="archived">archived</option></select></label><label className="sm:col-span-2"><span className="sb-label">Set notes</span><textarea aria-label={`Notes for setlist ${setlist.name}`} className="sb-input mt-1.5 min-h-20" value={notes} maxLength={2000} onChange={(event) => setNotes(event.target.value)} placeholder="Show-specific cues, tuning notes, or a backup plan" /></label></div><div className="mt-4 rounded-lg bg-[var(--surface-subtle)] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-medium">{summary.durationLabel}</p><p className="mt-1 text-xs text-[var(--text-muted)]">{summary.songCount} song{summary.songCount === 1 ? "" : "s"} · {summary.breakCount} break{summary.breakCount === 1 ? "" : "s"} · song time excludes breaks</p></div>{summary.timingStatus === "incomplete" ? <Badge variant="warning">timing incomplete</Badge> : summary.timingStatus === "timed" ? <Badge variant="success">timed</Badge> : <Badge variant="neutral">empty</Badge>}</div>{attachedEvents.length === 0 ? <button type="button" className="sb-btn-secondary mt-3" onClick={() => openWorkspace("events")}>Attach this set from Events</button> : null}</div><div className="mt-4 flex flex-col gap-2 sm:flex-row"><select aria-label={`Song to add to ${setlist.name}`} className="sb-select min-w-0 flex-1" value={songToAdd} onChange={(event) => setSongToAdd(event.target.value)}><option value="">Choose a song</option>{songs.filter((song) => song.active || items.some((item) => item.songId === song.id)).map((song) => <option key={song.id} value={song.id}>{song.title}{song.durationSeconds ? ` · ${formatSongDuration(song.durationSeconds)}` : " · duration unknown"}</option>)}</select><button type="button" className="sb-btn-secondary" disabled={busy || !songToAdd || items.length >= 100} aria-label={`Add song to ${setlist.name}`} onClick={addSong}><Plus className="h-4 w-4" /> Song</button><button type="button" className="sb-btn-secondary" disabled={busy || items.length >= 100} aria-label={`Add break to ${setlist.name}`} onClick={() => addMarker("break")}><Plus className="h-4 w-4" /> Break</button><button type="button" className="sb-btn-secondary" disabled={busy || items.length >= 100} aria-label={`Add note to ${setlist.name}`} onClick={() => addMarker("note")}><Plus className="h-4 w-4" /> Note</button></div><ol className="mt-4 space-y-2">{items.map((item, index) => { const song = item.songId ? songs.find((candidate) => candidate.id === item.songId) : null; return <li key={`${item.itemType}-${index}`} className="rounded-lg border border-[var(--border)] p-3"><div className="flex items-start gap-3"><span className="mt-2 w-6 shrink-0 text-center text-xs font-semibold text-[var(--text-muted)]">{index + 1}</span><div className="min-w-0 flex-1 space-y-2">{item.itemType === "song" ? <select aria-label={`Song at position ${index + 1} in ${setlist.name}`} className="sb-select" value={item.songId ?? ""} onChange={(event) => updateItem(index, { songId: event.target.value || null })}><option value="">Choose a saved song</option>{songs.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.title}{!candidate.active ? " · inactive" : ""}</option>)}</select> : <input aria-label={`${item.itemType === "break" ? "Break" : "Note"} at position ${index + 1} in ${setlist.name}`} className="sb-input" value={item.label} maxLength={240} onChange={(event) => updateItem(index, { label: event.target.value })} />}<input aria-label={`Transition after position ${index + 1} in ${setlist.name}`} className="sb-input" value={item.transitionNotes} maxLength={1000} onChange={(event) => updateItem(index, { transitionNotes: event.target.value })} placeholder={item.itemType === "song" ? "Transition, tuning, count-in, or segue" : "Optional detail"} /><p className="text-xs text-[var(--text-muted)]">{item.itemType === "song" ? song?.durationSeconds ? `${song.title} · ${formatSongDuration(song.durationSeconds)}` : `${song?.title ?? "Song"} · duration unknown` : item.itemType}</p></div><div className="flex shrink-0 flex-col gap-1"><button type="button" className="sb-btn-ghost px-2" aria-label={`Move position ${index + 1} up in ${setlist.name}`} disabled={busy || index === 0} onClick={() => move(index, -1)}><ArrowUp className="h-4 w-4" /></button><button type="button" className="sb-btn-ghost px-2" aria-label={`Move position ${index + 1} down in ${setlist.name}`} disabled={busy || index === items.length - 1} onClick={() => move(index, 1)}><ArrowDown className="h-4 w-4" /></button><button type="button" className="sb-btn-ghost px-2 text-red-300" aria-label={`Remove position ${index + 1} from ${setlist.name}`} disabled={busy} onClick={() => setItems((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 className="h-4 w-4" /></button></div></div></li>; })}</ol>{!items.length ? <p className="mt-4 text-sm text-[var(--text-muted)]">No running order yet. Add a song, break, or note above.</p> : null}{!setlist.updatedAt ? <p role="alert" className="mt-4 text-sm text-amber-200">Refresh before saving. StoryBoard could not verify which running-order version you opened.</p> : null}<button type="button" className="sb-btn-primary mt-4" disabled={busy || !valid || !setlist.updatedAt} onClick={() => { void create(`/setlists/${setlist.id}`, { expectedUpdatedAt: setlist.updatedAt, name, status, notes: notes.trim() || null, items: items.map((item) => ({ songId: item.itemType === "song" ? item.songId : null, itemType: item.itemType, label: item.label.trim() || null, transitionNotes: item.transitionNotes.trim() || null })) }, "PATCH"); }}><Save className="h-4 w-4" /> Save running order</button></div></details>;
}
