...
export function ManagerClient({ initialProfile, initialMembers, initialMemberCheckIns, initialGoals, initialGoalMeasurements, initialDecisions, initialBrief, initialConversations, initialConversation, initialMemory, initialLearning, initialResponseReview, initialResponseEvalReview, initialPlanHealth, initialContextHealth, initialKnowledgeHealth, initialEvidenceHealth, initialGoalPath, initialWorkSequence, initialCommitmentHealth, initialTeamLoad, initialOutcomeReview, initialEvalExamples, initialResponseEvalExamples, initialEvaluation, initialSettings, initialProviderContextPolicy, isOwner }: { initialProfile: ManagerProfile | null; initialMembers: BandMember[]; initialMemberCheckIns: BandMemberCheckIn[]; initialGoals: ManagerGoal[]; initialGoalMeasurements: ManagerGoalMeasurement[]; initialDecisions: ManagerDecision[]; initialBrief: ManagerRun | null; initialConversations: ManagerConversationSummary[]; initialConversation: ManagerConversation | null; initialMemory: ManagerMemoryFact[]; initialLearning: ManagerLearningSummary | null; initialResponseReview: ManagerResponseReviewQueue | null; initialResponseEvalReview: ManagerResponseEvalReviewQueue | null; initialPlanHealth: ManagerPlanHealth | null; initialContextHealth: ManagerContextHealth | null; initialKnowledgeHealth: ManagerKnowledgeHealth | null; initialEvidenceHealth: ManagerEvidenceHealth | null; initialGoalPath: ManagerGoalPath | null; initialWorkSequence: ManagerWorkSequence | null; initialCommitmentHealth: ManagerCommitmentHealth | null; initialTeamLoad: ManagerTeamLoad | null; initialOutcomeReview: ManagerOutcomeReview | null; initialEvalExamples: ManagerEvalExample[] | null; initialResponseEvalExamples: ManagerResponseEvalExample[] | null; initialEvaluation: ManagerEvaluationRun | null; initialSettings: ManagerSettings | null; initialProviderContextPolicy: ManagerProviderContextPolicy | null; isOwner: boolean }) {
  ...
  async function promoteResponseEval(messageId: string, payload: { label: "useful" | "needs_revision"; expectedBehavior?: string | null }) {
    ...
    try {
      ...
      const refreshed = await apiFetch<ManagerResponseEvalReviewQueue>("/manager/response-eval-review?limit=3").catch(() => null);
      if (refreshed) setResponseEvalReview(refreshed);
    } catch (err) { setError(err instanceof Error ? err.message : "Request failed"); throw err; } finally { setBusy(false); }
  }
  ...
  async function submitMessageFeedback(messageId: string, payload: { helpful: boolean; reason?: string | null; note?: string | null }) {
    ...
    try {
      ...
      const [learningResult, reviewResult, evalReviewResult] = await Promise.allSettled([apiFetch<ManagerLearningSummary>("/manager/learning"), apiFetch<ManagerResponseReviewQueue>("/manager/response-review?limit=3"), isOwner ? apiFetch<ManagerResponseEvalReviewQueue>("/manager/response-eval-review?limit=3") : Promise.resolve(null)]);
...
function GoalProgressCard({ goal, measurement, health, busy, onRecord, onMeasurementKind, onTargetDirection, onSync }: { goal: ManagerGoal; measurement: ManagerGoalMeasurement | null; health: ManagerPlanHealth["goals"][number] | null; busy: boolean; onRecord: (goalId: string, value: number, note: string | null) => Promise<void>; onMeasurementKind: (goalId: string, kind: ManagerGoalMeasurementKind) => Promise<void>; onTargetDirection: (goalId: string, direction: ManagerGoalTargetDirection) => Promise<void>; onSync: (goalId: string, observedValue: number) => Promise<void> }) {
  ...
  return <div className="rounded-lg border border-[var(--border)] p-3">
    ...
    {editing ? <div className="mt-3 space-y-2 rounded-lg bg-[var(--surface-0)] p-3"><label><span className="sb-label">Current value</span><input className="sb-input mt-1" type="number" step="any" value={value} onChange={(event) => setValue(event.target.value)} /></label><label><span className="sb-label">What changed? (optional)</span><input className="sb-input mt-1" value={note} maxLength={1000} onChange={(event) => setNote(event.target.value)} placeholder="Booked the second regional show" /></label><div className="flex gap-2"><button className="sb-btn-primary" disabled={busy || !Number.isFinite(Number(value))} onClick={() => void save()}><Save className="h-4 w-4" /> Record</button><button className="sb-btn-ghost" disabled={busy} onClick={() => { setValue(goal.currentValue?.toString() ?? ""); setNote(""); setEditing(false); }}>Cancel</button></div></div> : <button className="sb-btn-ghost mt-2" disabled={busy} onClick={() => setEditing(true)}>Update progress</button>}
  </div>;
}

function MemoryFactEditor({ fact, assessment, busy, onSave, onConfirm, onArchive }: { fact: ManagerMemoryFact; assessment: ManagerKnowledgeHealth["items"][number] | null; busy: boolean; onSave: (value: unknown) => Promise<void>; onConfirm: () => Promise<void>; onArchive: () => Promise<void> }) {
  const [editing, setEditing] = useState(false);
