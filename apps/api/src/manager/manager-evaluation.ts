...
function goldenResults(candidateVersion: string): EvalResult[] {
  ...
  return [
    ...
    { name: "vault-import-catalog-stays-record-bound", source: "golden", passed: /Harbor Lights/.test(vaultImportAnswer.answer) && /Vault setlist-ready/.test(vaultImportAnswer.answer) && /not a fourth live band/i.test(vaultImportAnswer.answer) && vaultImportAnswer.citations.includes("vault:catalog_import_v1:RD-0001") && !/parked demo|trailer sketch|dirty sketch|travis|pitch/i.test(vaultImportAnswer.answer) && vaultImportAnswer.recommendation === null, detail: "A Vault import plan answers only live-band StoryBoard rows and does not invent parked titles or a booking pitch." },
