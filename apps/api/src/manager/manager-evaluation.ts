...
export const MANAGER_EVAL_DATASET_VERSION = "manager_evals_v39";
...
function goldenResults(candidateVersion: string): EvalResult[] {
  ...
  return [
    ...
    { name: "empty-seed-setlist-honesty", source: "golden", passed: /no songs or setlists are recorded/i.test(emptySetlistAnswer.answer) && /will not invent a catalog/i.test(emptySetlistAnswer.answer) && /AI-Music-Vault|app_api|master_catalog|catalog:import/i.test(emptySetlistAnswer.answer) && /never fetches a remote catalog/i.test(emptySetlistAnswer.answer) && /--apply/i.test(emptySetlistAnswer.answer) && /rad dad/i.test(emptySetlistAnswer.answer) && /not a second catalog/i.test(emptySetlistAnswer.answer) && !/opener|closer|travis|harbor lights/i.test(emptySetlistAnswer.answer), detail: "An empty song/event seed does not invent a setlist or catalog and points at local Vault import (Rad Dad default, dry-run, apply behind a flag, no remote fetch)." },
    ...
    { name: "live-vault-default-import-stays-empty", source: "golden", passed: liveVaultHonestyPlan.songs.length === 0 && liveVaultHonestyPlan.setlists.length === 0 && /no songs or setlists are recorded/i.test(liveVaultEmptyAnswer.answer) && /will not invent a catalog/i.test(liveVaultEmptyAnswer.answer) && /rad dad/i.test(liveVaultEmptyAnswer.answer) && !/harbor lights|sidewalk radio|parked demo/i.test(liveVaultEmptyAnswer.answer) && liveVaultEmptyAnswer.recommendation === null, detail: "A live Vault feed with writer, hybrid, and parked rows and an empty setlist_ready_default_import stays empty. Manager does not invent titles from played_live or Jeff Story." },
    { name: "writer-project-is-not-live-band", source: "golden", passed: /writer project/i.test(writerProjectAnswer.answer) && /rad dad/i.test(writerProjectAnswer.answer) && /not a fourth live band/i.test(writerProjectAnswer.answer) && writerProjectAnswer.recommendation === null && !/harbor lights|sidewalk radio/i.test(writerProjectAnswer.answer), detail: "Jeff Story is a writer project. Manager will not invent a live catalog or a fourth live band from those rows." },
