...
export const MANAGER_EVAL_DATASET_VERSION = "manager_evals_v41";
...
function goldenResults(candidateVersion: string): EvalResult[] {
  ...
  return [
    ...
    { name: "recorded-catalog-stays-record-bound", source: "golden", passed: /1 song/.test(recordedCatalogAnswer.answer) && /Harbor Lights/.test(recordedCatalogAnswer.answer) && recordedCatalogAnswer.citations.includes("song-harbor") && !/travis|pitch/i.test(recordedCatalogAnswer.answer), detail: "A recorded song library is answered from StoryBoard rows only and does not invent booking targets." },
