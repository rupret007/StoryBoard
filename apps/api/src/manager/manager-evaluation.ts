...
function goldenResults(candidateVersion: string): EvalResult[] {
  ...
  return [
    ...
    { name: "empty-seed-setlist-honesty", source: "golden", passed: /no songs or setlists are recorded/i.test(emptySetlistAnswer.answer) && /will not invent a catalog/i.test(emptySetlistAnswer.answer) && /AI-Music-Vault|app_api|master_catalog|catalog:import/i.test(emptySetlistAnswer.answer) && !/opener|closer|travis|rad dad|harbor lights/i.test(emptySetlistAnswer.answer), detail: "An empty song/event seed does not invent a setlist or catalog and points at Vault import." },
