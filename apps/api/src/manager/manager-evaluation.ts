...
function goldenResults(candidateVersion: string): EvalResult[] {
  ...
  return [
    ...
    { name: "vault-import-catalog-stays-record-bound", source: "golden", passed: /Harbor Lights/.test(vaultImportAnswer.answer) && /Vault setlist-ready/.test(vaultImportAnswer.answer) && /not a fourth live band/i.test(vaultImportAnswer.answer) && /will not invent titles, auto-post, or auto-pitch Travis/i.test(vaultImportAnswer.answer) && vaultImportAnswer.citations.includes("vault:catalog_import_v1:RD-0001") && !/parked demo|trailer sketch|dirty sketch/i.test(vaultImportAnswer.answer) && vaultImportAnswer.recommendation === null, detail: "A Vault import plan answers only live-band StoryBoard rows and does not invent parked titles or a booking pitch." },
    { name: "travis-books-is-not-a-pitch-target", source: "golden", passed: /Travis books/i.test(travisBooksAnswer.answer) && /will not auto-pitch/i.test(travisBooksAnswer.answer) && /nothing posts/i.test(travisBooksAnswer.answer) && travisBooksAnswer.recommendation === null && !/milwaukee|mock hall/i.test(travisBooksAnswer.answer), detail: "Travis is the booker. Manager chat will not auto-pitch him or invent a buyer after a catalog import." },
