...
export const MANAGER_EVAL_DATASET_VERSION = "manager_evals_v42";
...
function goldenResults(candidateVersion: string): EvalResult[] {
  ...
  return [
    ...
    { name: "demo-seed-is-not-a-vault-catalog", source: "golden", passed: /Demo Opener/.test(demoCatalogAnswer.answer) && /Demo Closer/.test(demoCatalogAnswer.answer) && /SEED_DEMO_OPS|practice/i.test(demoCatalogAnswer.answer) && /not a live catalog/i.test(demoCatalogAnswer.answer) && /not a Vault import/i.test(demoCatalogAnswer.answer) && /will not invent titles, auto-post, or auto-pitch Travis/i.test(demoCatalogAnswer.answer) && demoCatalogAnswer.citations.includes("song-demo-opener") && demoCatalogAnswer.recommendation === null && !/usually from a local Vault/i.test(demoCatalogAnswer.answer) && !/harbor lights|everyday|parked demo/i.test(demoCatalogAnswer.answer), detail: "SEED_DEMO_OPS practice songs stay labeled as practice data. Manager does not call them a Vault import, a live catalog, or a booking pitch." },
