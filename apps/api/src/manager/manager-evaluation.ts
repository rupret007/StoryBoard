...
function goldenResults(candidateVersion: string): EvalResult[] {
  ...
  return [
    ...
    { name: "empty-seed-booking-does-not-pitch", source: "golden", passed: /0 active opportunities/i.test(emptyPitchAnswer.answer) && /no recorded booking action/i.test(emptyPitchAnswer.answer) && !/travis|rad dad|mock hall/i.test(emptyPitchAnswer.answer) && (!emptyPitchAnswer.recommendation?.proposedAction || emptyPitchAnswer.recommendation.proposedAction.type === "create_task"), detail: "Empty booking records stay honest and never auto-pitch a buyer or invent a venue." },
