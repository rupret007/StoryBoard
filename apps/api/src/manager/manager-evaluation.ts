...
export const MANAGER_EVAL_DATASET_VERSION = "manager_evals_v40";
...
function goldenResults(candidateVersion: string): EvalResult[] {
  ...
  return [
    ...
    { name: "vault-import-catalog-stays-record-bound", source: "golden", passed: /Harbor Lights/.test(vaultImportAnswer.answer) && /Vault setlist-ready/.test(vaultImportAnswer.answer) && vaultImportAnswer.citations.includes("vault:catalog_import_v1:RD-0001") && !/parked demo|trailer sketch|dirty sketch|travis|pitch/i.test(vaultImportAnswer.answer) && vaultImportAnswer.recommendation === null, detail: "A Vault import plan answers only live-band StoryBoard rows and does not invent parked titles or a booking pitch." },
    ...
    { name: "parked-catalog-is-not-a-fourth-live-band", source: "golden", passed: /current artist only/i.test(fourthBandAnswer.answer) && /Rad Dad/i.test(fourthBandAnswer.answer) && /not a fourth live band/i.test(fourthBandAnswer.answer) && /parked/i.test(fourthBandAnswer.answer) && fourthBandAnswer.recommendation === null && !/dirty sketch|harbor lights/i.test(fourthBandAnswer.answer), detail: "Parked catalogs stay parked on the current artist. Something Dirty is not a fourth live band." },
    ...
    { name: "live-vault-default-import-uses-published-slice", source: "golden", passed: liveVaultPublishedPlan.songs.map((song) => song.title).join(",") === "Harbor Lights,Sidewalk Radio" && liveVaultPublishedPlan.setlists[0]?.items.length === 2 && /Harbor Lights/.test(liveVaultPublishedAnswer.answer) && /Sidewalk Radio/.test(liveVaultPublishedAnswer.answer) && /Vault setlist-ready/.test(liveVaultPublishedAnswer.answer) && !/parked demo/i.test(liveVaultPublishedAnswer.answer) && liveVaultPublishedAnswer.recommendation === null, detail: "When Vault publishes a default-live slice, default import and Manager list those recorded rows only — including Jeff Story and hybrid Rad Dad plays — and leave parked titles out." },
