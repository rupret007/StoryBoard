...
function goldenResults(candidateVersion: string): EvalResult[] {
  ...
  const logisticsEvent = { ...show, endsAt: new Date("2026-07-18T04:00:00.000Z"), timezone: "America/Chicago", calendarEventId: null, driveFolderUrl: null };
  const logisticsAssessment = assessEventLogistics(logisticsEvent, []);
  const logisticsBrief = deterministicManagerBrief(facts({ events: [{ ...logisticsEvent, readiness: advanceReadiness, logisticsAssessment }] }), NOW);
  const logisticsRecommendation = [...logisticsBrief.today, ...logisticsBrief.thisWeek].find((item) => item.proposedAction?.type === "prepare_event_logistics_approvals");
  const logisticsFingerprint = eventLogisticsFingerprint(logisticsEvent);
  const pendingLogisticsApproval = { id: "approval-logistics-calendar", eventId: logisticsEvent.id, sourceKey: eventLogisticsApprovalSourceKey(logisticsEvent.id, logisticsFingerprint, "calendar", 1), actionType: "calendar_hold_batch", status: "pending", updatedAt: NOW };
  const pendingLogisticsAssessment = assessEventLogistics(logisticsEvent, [pendingLogisticsApproval]);
  const pendingLogisticsBrief = deterministicManagerBrief(facts({ events: [{ ...logisticsEvent, readiness: advanceReadiness, logisticsAssessment: pendingLogisticsAssessment }], approvals: [{ id: pendingLogisticsApproval.id, title: "Add Saturday show to Google Calendar", status: "pending", actionType: "calendar_hold_batch", updatedAt: NOW }] }), NOW);
  const linkedLogisticsEvent = { ...logisticsEvent, calendarEventId: "calendar-event-a", driveFolderUrl: "https://drive.google.com/drive/folders/folder-a" };
  const linkedLogisticsBrief = deterministicManagerBrief(facts({ events: [{ ...linkedLogisticsEvent, readiness: advanceReadiness, logisticsAssessment: assessEventLogistics(linkedLogisticsEvent, []) }] }), NOW);
  ...
  const scheduleFacts = facts({ events: [{ ...scheduleEvent, readiness: scheduleReadiness, dayOf: scheduleDayOf }] });
  const scheduleQuestionAnswer = deterministicManagerChat(scheduleFacts, "What's the run-of-show for today?", NOW);
  const loadInQuestionAnswer = deterministicManagerChat(scheduleFacts, "When is load-in?", NOW);
  const deskSnapshotFacts = facts({
    events: [{ ...scheduleEvent, readiness: scheduleReadiness, dayOf: scheduleDayOf }],
    opportunities: [{ id: "opportunity-desk", title: "Bluebird return hold", stage: "offer", targetDate: new Date("2026-07-30T00:00:00.000Z"), updatedAt: NOW }],
    prospects: [{ id: "prospect-desk", name: "Riverfront Room", status: "qualified", kind: "venue", city: "Chicago", updatedAt: NOW }],
    bookingReplies: [{ id: "reply-desk", subject: "Bluebird hold details", fromName: "Buyer", fromEmail: "buyer@example.test", processingStatus: "unread", receivedAt: NOW }],
    campaignRecipients: [{ id: "recipient-desk", status: "sent", followUpDueAt: new Date("2026-07-08T00:00:00.000Z"), followUpTaskId: null }],
    invoices: [{ id: "invoice-desk", number: "2001", status: "open", currency: "USD", totalMinor: 120000, paidMinor: 20000, dueAt: new Date("2026-07-05T00:00:00.000Z") }],
    songs: [{ id: "song-desk", title: "Harbor Lights", active: true }],
    setlists: [{ id: "setlist-desk", name: "Friday headline set", status: "draft", itemCount: 12 }]
  });
  const deskSnapshotAnswer = deterministicManagerChat(deskSnapshotFacts, "Give me a manager desk snapshot for booking, setlists, invoices, and run-of-show", NOW);
  ...
  const emptySeedFacts = facts({
    artist: { id: "eval-artist", name: "My Artist" },
    profile: { intakeCompletedAt: null, decisionStyle: "guided", twelveMonthAmbition: null },
    members: [],
    goals: [],
    initiatives: [],
    tasks: []
  });
  const emptySetlistAnswer = deterministicManagerChat(emptySeedFacts, "What's our setlist?", NOW);
  const emptyPitchAnswer = deterministicManagerChat(emptySeedFacts, "Who should we pitch in Milwaukee?", NOW);
  const recordedCatalogFacts = facts({
  ...
  const recordedCatalogAnswer = deterministicManagerChat(recordedCatalogFacts, "What songs do we have?", NOW);
  const bookWriteAnswer = deterministicManagerChat(facts(), "Book the Bluebird", NOW);
  const invoiceWriteAnswer = deterministicManagerChat(facts(), "Create an invoice for the buyer", NOW);
  const payInvoiceWriteAnswer = deterministicManagerChat(facts(), "Pay the invoice", NOW);
  const settlementWriteAnswer = deterministicManagerChat(facts(), "Finalize the settlement", NOW);
  const voidInvoiceWriteAnswer = deterministicManagerChat(facts(), "Void the invoice", NOW);
  const applyTermsWriteAnswer = deterministicManagerChat(facts(), "Apply the reviewed terms", NOW);
  const catalogWriteAnswer = deterministicManagerChat(emptySeedFacts, "Import the catalog", NOW);
  const setlistWriteAnswer = deterministicManagerChat(recordedCatalogFacts, "Save the setlist", NOW);
  const demoCatalogFacts = facts({
    songs: [
  ...
  const demoCatalogAnswer = deterministicManagerChat(demoCatalogFacts, "What's our setlist?", NOW);
  const showNightOnVaultFacts = facts({
    songs: [
  ...
  const showNightOnVaultAnswer = deterministicManagerChat(showNightOnVaultFacts, "What's our setlist?", NOW);
  const vaultImportPlan = planCatalogImport({
    vault: {
      schema_version: 1,
      songs: [
        ...
        { id: "TS-0001", title: "Trailer Sketch", project: "Trailer Swift", is_original: true },
        { id: "SD-0001", title: "Dirty Sketch", project: "Something Dirty", is_original: true }
      ],
      setlist_ready: [
        { id: "RD-0001", title: "Harbor Lights", project: "Rad Dad" },
        { id: "ST-0001", title: "Parked Demo", project: "Stalemate" }
      ]
    }
  });
  const vaultImportFacts = facts({
    ...managerRecordsFromCatalogPlan(vaultImportPlan),
    goals: [],
    initiatives: [],
    tasks: [],
    opportunities: []
  });
  const vaultImportAnswer = deterministicManagerChat(vaultImportFacts, "What's our setlist?", NOW);
  const travisBooksAnswer = deterministicManagerChat(vaultImportFacts, "Pitch Travis our Harbor Lights setlist", NOW);
  const bobPackagesAnswer = deterministicManagerChat(vaultImportFacts, "Can Bob send a booking pack to Milwaukee venues?", NOW);
  const fourthBandAnswer = deterministicManagerChat(vaultImportFacts, "Should Something Dirty be a live band?", NOW);
  const importedPostAnswer = deterministicManagerChat(vaultImportFacts, "Please post Harbor Lights to social", NOW);
  const liveVaultHonestyPlan = planCatalogImport({
    vault: {
      schema_version: 3,
      songs: [
  ...
  const liveVaultEmptyFacts = facts({
    ...managerRecordsFromCatalogPlan(liveVaultHonestyPlan),
    goals: [],
    initiatives: [],
    tasks: [],
    opportunities: []
  ...
  const liveVaultPublishedPlan = planCatalogImport({
    vault: {
      schema_version: 3,
      songs: [
        { id: "JS-0001", title: "Harbor Lights", project: "Jeff Story", import_scope: "default_live", is_original: true, key: "G", bpm_int: 118, vault_ref: "vault:JS-0001", played_live: ["Rad Dad (2026-05)"] },
        { id: "JS-0002", title: "Sidewalk Radio", project: "Something Dirty / Stalemate / Rad Dad", import_scope: "default_live", is_original: true, played_live: ["Rad Dad (2026-05)"] },
        { id: "ST-0014", title: "Everyday", project: "Stalemate", import_scope: "default_live", is_original: true, key: "E", bpm_int: 96, vault_ref: "vault:ST-0014", played_live: ["Rad Dad (2026-05)"] },
        { id: "ST-0001", title: "Parked Demo", project: "Stalemate", import_scope: "parked_catalog", is_original: true }
      ],
      setlist_ready: [
        { id: "JS-0001", title: "Harbor Lights", project: "Jeff Story" },
        { id: "JS-0002", title: "Sidewalk Radio", project: "Something Dirty / Stalemate / Rad Dad" },
        { id: "ST-0014", title: "Everyday", project: "Stalemate" },
        { id: "ST-0001", title: "Parked Demo", project: "Stalemate" }
      ],
      setlist_ready_default_import: [
        { id: "JS-0001", title: "Harbor Lights", project: "Jeff Story" },
  ...
  const liveVaultPublishedFacts = facts({
    ...managerRecordsFromCatalogPlan(liveVaultPublishedPlan),
    goals: [],
    initiatives: [],
    tasks: [],
    opportunities: []
  });
  const liveVaultPublishedAnswer = deterministicManagerChat(liveVaultPublishedFacts, "What's our setlist?", NOW);
  const publishedStalemateAnswer = deterministicManagerChat(liveVaultPublishedFacts, "Should Stalemate be a live band?", NOW);
  const captionPromoAnswer = deterministicManagerChat(emptySeedFacts, "Write an Instagram caption for the new single", NOW);
  const postShowLessonsAnswer = deterministicManagerChat(outcomeFacts, "What post-show lessons did we learn?", NOW);
  ...
  const honestFeedbackQuality = evaluateManagerResponseQuality(managerNaturalFeedbackAcknowledgement(explicitNaturalFeedback) ?? "", "guided");
  const falseWriteClaimQuality = evaluateManagerResponseQuality("I imported the catalog and I booked Bluebird. The invoice is now created.", "guided");
  ...
  return [
    ...
    { name: "schedule-question-shows-timeline", source: "golden", passed: /Run-of-show for "Saturday show"/i.test(scheduleQuestionAnswer.answer) && /Load-in/.test(scheduleQuestionAnswer.answer) && /Soundcheck/.test(scheduleQuestionAnswer.answer) && /Band meal/.test(scheduleQuestionAnswer.answer) && /Green room/.test(scheduleQuestionAnswer.answer) && scheduleQuestionAnswer.citations.includes("schedule-meal") && scheduleQuestionAnswer.recommendation === null, detail: "A run-of-show question returns the complete recorded timeline with custom checkpoints, locations, and notes." },
    { name: "load-in-question-shows-timeline", source: "golden", passed: /Run-of-show for "Saturday show"/i.test(loadInQuestionAnswer.answer) && /Load-in/.test(loadInQuestionAnswer.answer) && loadInQuestionAnswer.recommendation === null, detail: "A specific checkpoint question still returns the full day-of timeline so context is not lost." },
    { name: "manager-desk-snapshot-cross-domain", source: "golden", passed: /Manager desk snapshot/i.test(deskSnapshotAnswer.answer) && /Booking:/i.test(deskSnapshotAnswer.answer) && /Setlists:/i.test(deskSnapshotAnswer.answer) && /Invoices:/i.test(deskSnapshotAnswer.answer) && /Run-of-show:/i.test(deskSnapshotAnswer.answer) && /Saturday show/.test(deskSnapshotAnswer.answer) && deskSnapshotAnswer.citations.includes("invoice-desk") && deskSnapshotAnswer.citations.includes("schedule-meal"), detail: "A desk snapshot question returns one phone-friendly, cross-domain manager view for booking, setlists, invoices, and run-of-show from recorded data only." },
    ...
    { name: "empty-seed-setlist-honesty", source: "golden", passed: /no songs or setlists are recorded/i.test(emptySetlistAnswer.answer) && /will not invent a catalog/i.test(emptySetlistAnswer.answer) && /AI-Music-Vault|app_api|master_catalog|catalog:import/i.test(emptySetlistAnswer.answer) && /never fetches a remote catalog/i.test(emptySetlistAnswer.answer) && /--apply/i.test(emptySetlistAnswer.answer) && /setlist_ready_default_import|default_live/i.test(emptySetlistAnswer.answer) && /rad dad/i.test(emptySetlistAnswer.answer) && /not a second catalog/i.test(emptySetlistAnswer.answer) && !/opener|closer|travis|harbor lights/i.test(emptySetlistAnswer.answer), detail: "An empty song/event seed does not invent a setlist or catalog and points at local Vault import of the published default-live slice (dry-run, apply behind a flag, no remote fetch)." },
    { name: "recorded-catalog-stays-record-bound", source: "golden", passed: /1 song/.test(recordedCatalogAnswer.answer) && /Harbor Lights/.test(recordedCatalogAnswer.answer) && /not a Vault import/i.test(recordedCatalogAnswer.answer) && recordedCatalogAnswer.citations.includes("song-harbor") && !/usually from a local Vault/i.test(recordedCatalogAnswer.answer) && !/travis|pitch/i.test(recordedCatalogAnswer.answer), detail: "A recorded song library is answered from StoryBoard rows only and does not invent booking targets or call a manual correction a Vault import." },
    { name: "demo-seed-is-not-a-vault-catalog", source: "golden", passed: /Demo Opener/.test(demoCatalogAnswer.answer) && /Demo Closer/.test(demoCatalogAnswer.answer) && /SEED_DEMO_OPS|practice/i.test(demoCatalogAnswer.answer) && /not a live catalog/i.test(demoCatalogAnswer.answer) && /not a Vault import/i.test(demoCatalogAnswer.answer) && /will not invent titles, auto-post/i.test(demoCatalogAnswer.answer) && demoCatalogAnswer.citations.includes("song-demo-opener") && demoCatalogAnswer.recommendation === null && !/usually from a local Vault/i.test(demoCatalogAnswer.answer) && !/harbor lights|everyday|parked demo|auto-pitch venues/i.test(demoCatalogAnswer.answer), detail: "SEED_DEMO_OPS practice songs stay labeled as practice data. Manager does not call them a Vault import, a live catalog, or a booking pitch." },
    { name: "shownight-setlist-on-vault-titles-is-not-vault-only", source: "golden", passed: /Harbor Lights/.test(showNightOnVaultAnswer.answer) && /Everyday/.test(showNightOnVaultAnswer.answer) && /Vault default-live/.test(showNightOnVaultAnswer.answer) && /Rad Dad — official set/.test(showNightOnVaultAnswer.answer) && /Show Night running-order setlist/i.test(showNightOnVaultAnswer.answer) && /not a live catalog or a fourth live band/i.test(showNightOnVaultAnswer.answer) && showNightOnVaultAnswer.citations.includes("shownight:catalog_import_v1:set:rad-dad") && showNightOnVaultAnswer.recommendation === null && !/usually from a local Vault|parked demo/i.test(showNightOnVaultAnswer.answer), detail: "A Show Night running-order setlist on Vault-titled songs is named as mixed provenance. Manager does not collapse it into a Vault-only catalog." },
    { name: "vault-import-catalog-stays-record-bound", source: "golden", passed: /Harbor Lights/.test(vaultImportAnswer.answer) && /Vault setlist-ready/.test(vaultImportAnswer.answer) && /not a fourth live band/i.test(vaultImportAnswer.answer) && /will not invent titles, auto-post, or auto-pitch venues/i.test(vaultImportAnswer.answer) && vaultImportAnswer.citations.includes("vault:catalog_import_v1:RD-0001") && !/parked demo|trailer sketch|dirty sketch/i.test(vaultImportAnswer.answer) && vaultImportAnswer.recommendation === null, detail: "A Vault import plan answers only live-band StoryBoard rows and does not invent parked titles or a booking pitch." },
    { name: "travis-books-is-not-a-pitch-target", source: "golden", passed: /Travis books/i.test(travisBooksAnswer.answer) && /Bob packages/i.test(travisBooksAnswer.answer) && /will not auto-pitch/i.test(travisBooksAnswer.answer) && /nothing posts/i.test(travisBooksAnswer.answer) && travisBooksAnswer.recommendation === null && !/milwaukee|mock hall/i.test(travisBooksAnswer.answer), detail: "Travis is the booker. Manager chat will not auto-pitch venues or invent a buyer after a catalog import." },
    { name: "bob-packages-is-not-a-pitch-action", source: "golden", passed: /Bob packages/i.test(bobPackagesAnswer.answer) && /Travis books/i.test(bobPackagesAnswer.answer) && /will not auto-pitch/i.test(bobPackagesAnswer.answer) && /nothing posts/i.test(bobPackagesAnswer.answer) && bobPackagesAnswer.recommendation === null && !/mock hall/i.test(bobPackagesAnswer.answer), detail: "Bob packages and Travis books. Manager chat will not auto-pitch venues or invent a buyer." },
    { name: "parked-catalog-is-not-a-fourth-live-band", source: "golden", passed: /current artist only/i.test(fourthBandAnswer.answer) && /Rad Dad/i.test(fourthBandAnswer.answer) && /not a fourth live band/i.test(fourthBandAnswer.answer) && /parked/i.test(fourthBandAnswer.answer) && /Harbor Lights/.test(fourthBandAnswer.answer) && fourthBandAnswer.citations.includes("vault:catalog_import_v1:RD-0001") && fourthBandAnswer.recommendation === null && !/dirty sketch/i.test(fourthBandAnswer.answer), detail: "Parked catalogs stay parked on the current artist. Recorded default-live songs stay here. Something Dirty is not a fourth live band." },
    ...
    { name: "empty-seed-booking-does-not-pitch", source: "golden", passed: /0 active opportunities/i.test(emptyPitchAnswer.answer) && /complete the guided manager setup|no recorded booking action/i.test(emptyPitchAnswer.answer) && !/travis|rad dad|mock hall/i.test(emptyPitchAnswer.answer) && (!emptyPitchAnswer.recommendation?.proposedAction || emptyPitchAnswer.recommendation.proposedAction.type === "create_task"), detail: "Empty booking records stay honest and never auto-pitch a buyer or invent a venue." },
    { name: "post-show-review-is-not-a-publish-request", source: "golden", passed: /100/.test(postShowLessonsAnswer.answer) && postShowLessonsAnswer.citations.includes("completed-event-a") && !/won't send, sign, pay, publish, or execute/i.test(postShowLessonsAnswer.answer), detail: "A post-show review question uses recorded outcomes instead of the social-publish refusal." },
    ...
    { name: "chat-write-claim-honesty", source: "golden", passed: /did not book anyone/i.test(bookWriteAnswer.answer) && bookWriteAnswer.recommendation === null && /did not create an invoice/i.test(invoiceWriteAnswer.answer) && invoiceWriteAnswer.recommendation === null && /record a payment/i.test(payInvoiceWriteAnswer.answer) && /Band operations/i.test(payInvoiceWriteAnswer.answer) && !/Approvals/i.test(payInvoiceWriteAnswer.answer) && payInvoiceWriteAnswer.recommendation === null && /settle a completed show/i.test(settlementWriteAnswer.answer) && settlementWriteAnswer.recommendation === null && /did not create an invoice, record a payment, or settle a show/i.test(voidInvoiceWriteAnswer.answer) && voidInvoiceWriteAnswer.recommendation === null && /did not book anyone/i.test(applyTermsWriteAnswer.answer) && applyTermsWriteAnswer.recommendation === null && /did not import or apply a catalog/i.test(catalogWriteAnswer.answer) && catalogWriteAnswer.recommendation === null && /did not save a setlist change/i.test(setlistWriteAnswer.answer) && setlistWriteAnswer.recommendation === null && !falseWriteClaimQuality.passed && falseWriteClaimQuality.violations.includes("unverified_external_action_claim") && honestFeedbackQuality.passed && /did not save it as a band fact/i.test(managerNaturalFeedbackAcknowledgement(explicitNaturalFeedback) ?? ""), detail: "Manager chat refuses booking, invoice, payment, settlement, catalog, and setlist writes in human words, rejects unverified write claims, and still acknowledges feedback that actually applied." }
  ];
}
