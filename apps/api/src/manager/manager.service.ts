...
import { MANAGER_WRITE_CLAIM_POLICY_VERSION, resolveManagerWriteClaim } from "./manager-write-claim";
...
export class ManagerService {
  ...
  private async facts(artistId: string) {
    const [
      ...
      this.prisma.client.song.findMany({ where: { artistId }, select: { id: true, title: true, active: true, musicalKey: true, sourceKey: true }, orderBy: [{ active: "desc" }, { title: "asc" }], take: 50 }),
      this.prisma.client.setlist.findMany({ where: { artistId }, select: { id: true, name: true, status: true, sourceKey: true, _count: { select: { items: true } } }, orderBy: { updatedAt: "desc" }, take: 20 }),
    ...
    return {
      ...
      invoices,
      songs,
      setlists: setlists.map((setlist) => ({ id: setlist.id, name: setlist.name, status: setlist.status, itemCount: setlist._count.items, sourceKey: setlist.sourceKey })),
      decisions,
  ...
  async chat(artistId: string, input: { conversationId?: string | null | undefined; message: string }, actorLabel: string, actorOperatorId: string, actorIsOwner = false) {
    ...
    const writeClaim = resolveManagerWriteClaim(input.message);
    const writeClaimRoute = !memoryCaptureRoute && naturalFeedback.status === "not_feedback" && !contextCaptureRoute && !taskCaptureRoute && !taskUpdateRoute && !taskAssignmentRoute && !projectCaptureRoute && !eventAvailabilityRoute && !eventCaptureRoute && writeClaim.status === "refused";
    ...
    const fallback = memoryCaptureRoute
      ...
      : writeClaimRoute
        ? { answer: writeClaim.message ?? "I did not change a booking, invoice, catalog, or setlist record from this chat.", citations: [], recommendation: null }
      : deterministicManagerChat(sharedFacts, input.message, now, continuity, subjectReference, responseAdaptation);
    ...
    let mode = memoryCaptureRoute ? "deterministic_memory_capture" : naturalFeedbackRoute ? "deterministic_feedback" : contextCaptureRoute ? "deterministic_context_capture" : taskCaptureRoute ? "deterministic_task_capture" : taskUpdateRoute ? "deterministic_task_update" : taskAssignmentRoute ? "deterministic_task_assignment" : projectCaptureRoute ? "deterministic_project_capture" : eventAvailabilityRoute ? "deterministic_event_availability" : eventCaptureRoute ? "deterministic_event_capture" : writeClaimRoute ? "deterministic_write_claim" : "deterministic";
    let model: string | null = null;
    let inputTokens: number | null = null;
    let outputTokens: number | null = null;
    ...
    const calibrated = memoryCaptureRoute || naturalFeedbackRoute || contextCaptureRoute || taskCaptureRoute || taskUpdateRoute || taskAssignmentRoute || projectCaptureRoute || eventAvailabilityRoute || eventCaptureRoute || writeClaimRoute
    ...
    const run = await this.prisma.client.managerRun.create({
      data: {
        ...
        trace: {
          ...
          toolsSelected: providerAttempted ? ["read_manager_snapshot"] : [],
          guardrails: ["known-evidence", "bounded-history", "structured-conversation-continuity", "tenant-subject-resolution", "explicit-natural-feedback", "reviewed-context-capture", "reviewed-task-capture", "reviewed-task-update", "reviewed-task-assignment", "reviewed-project-capture", "reviewed-event-capture", "reviewed-event-availability", "write-claim-honesty", "natural-response-quality", "internal-action-allowlist", "approval-boundary", "untrusted-record-text", "memory-sensitivity-policy", "authoritative-source-precedence", "knowledge-freshness", "operating-evidence-calibration", "task-prerequisite-sequencing", "goal-to-action-path", "goal-target-semantics", "code-owned-manager-coaching", "team-load-premise-check"],
          providerContext: {
            ...providerPolicy,
