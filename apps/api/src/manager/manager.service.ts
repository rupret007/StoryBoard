...
export class ManagerService {
  ...
  private async facts(artistId: string) {
    const [
      ...
      this.prisma.client.setlist.findMany({ where: { artistId }, select: { id: true, name: true, status: true, _count: { select: { items: true } } }, orderBy: { updatedAt: "desc" }, take: 20 }),
    ...
    return {
      ...
      invoices,
      songs,
      setlists: setlists.map((setlist) => ({ id: setlist.id, name: setlist.name, status: setlist.status, itemCount: setlist._count.items })),
      decisions,
