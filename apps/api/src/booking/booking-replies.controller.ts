...
export class BookingRepliesController {
  ...
  @Post(":id/prepare-confirmation") async prepareConfirmation(@Param("id") id: string, @CurrentOperator() op: RequestOperator, @Req() req: FastifyRequest, @Headers("x-artist-id") h?: string) { const artistId = await this.artistId(op.id, req, h); await this.roles.assertCanMutateWorkflow(op.id, artistId); return this.replies.prepareConfirmation(artistId, id, op.email, op.id); }
}
