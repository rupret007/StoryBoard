...
export class EventsController extends ArtistController {
  ...
  @Post(":id/live-set-position") async liveSetPosition(@Param("id") id: string, @Body() body: unknown, @CurrentOperator() op: RequestOperator, @Req() req: FastifyRequest, @Headers("x-artist-id") h?: string) { const artistId = await this.mutable(op, req, h); return this.operations.setLiveSetPosition(artistId, id, this.parse(eventLiveSetPositionSchema, body), op.email, op.id); }
  @Post(":id/after-show") async afterShow(@Param("id") id: string, @Body() body: unknown, @CurrentOperator() op: RequestOperator, @Req() req: FastifyRequest, @Headers("x-artist-id") h?: string) { const artistId = await this.mutable(op, req, h); return this.operations.recordAfterShow(artistId, id, this.parse(eventAfterShowPatchSchema, body), op.email, op.id); }
...
export class SongsController extends ArtistController {
  ...
  @Get("status") async status(@CurrentOperator() op: RequestOperator, @Req() req: FastifyRequest, @Headers("x-artist-id") h?: string) { return this.operations.catalogStatus(await this.artistId(op.id, req, h)); }
