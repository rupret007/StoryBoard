...
const [policy, pdf, managerSchemas, operationSchemas, operationsMod, managerMod, managerControllerMod, intelligence, responseQuality, outcomeReview, contextHealth, contextCapture, taskCapture, taskUpdate, taskAssignment, projectCapture, eventCapture, eventAvailability, knowledgeHealth, evidenceHealth, workSequence, goalPath, goalTarget, conversationContinuity, naturalFeedback, subjectReference, recommendationReview, responseReview, memoryCapture, goalMeasurement, coaching, commitmentHealth, teamLoad, managerSchedule, providerContext, tasksMod, evaluation, managerPlan, eventReadiness, eventDayOf, projectPlan, followThrough, workflowProcessorMod] = await Promise.all([
  ...
  loadApi("manager/manager-task-update.js"),
  loadApi("manager/manager-task-assignment.js"),
  loadApi("manager/manager-project-capture.js"),
  loadApi("manager/manager-event-capture.js"),
  loadApi("manager/manager-event-availability.js"),
...
test("Manager project capture stages one exact project and its dated milestone plan", () => {
  const captured = projectCapture.resolveManagerProjectCapture({ message: "Plan our EP release for October 15, 2026", sourceMessageId: "message-project", sourceMessageCreatedAt: now, projects: [] });
  assert.equal(captured.status, "ready");
  assert.equal(captured.action.type, "create_conversation_project");
  assert.equal(captured.action.projectType, "release");
  assert.equal(captured.action.name, "EP release");
  assert.equal(captured.action.dueDate, "2026-10-15");
  assert.equal(captured.action.planVersion, "project_plan_v1");
  assert.equal(projectCapture.managerConversationProjectDueAt(captured.action).toISOString(), "2026-10-15T12:00:00.000Z");
  assert.match(captured.preview, /Project: EP release/);
  assert.match(captured.preview, /Milestones \(6\)/);
  assert.match(captured.preview, /Complete masters, credits, and ownership details/);
  assert.equal(projectCapture.managerConversationProjectActionMatchesMessage(captured.action, { id: "message-project", content: "Plan our EP release for October 15, 2026", createdAt: now }), true);
  assert.equal(projectCapture.managerConversationProjectActionMatchesMessage(captured.action, { id: "message-other", content: "Plan our EP release for October 15, 2026", createdAt: now }), false);
  const recommendation = projectCapture.managerConversationProjectRecommendation(captured.action);
  assert.equal(recommendation.proposedAction.type, "create_conversation_project");
  assert.equal(recommendation.workstream, "releases");
  assert.deepEqual(recommendation.evidenceIds, []);

  const namedTour = projectCapture.resolveManagerProjectCapture({ message: 'Create a tour project called "Midwest Weekender" due 2026-11-20', sourceMessageId: "tour-message", sourceMessageCreatedAt: now, projects: [] });
  assert.equal(namedTour.action.projectType, "tour");
  assert.equal(namedTour.action.name, "Midwest Weekender");
  assert.equal(projectCapture.managerConversationProjectRecommendation(namedTour.action).workstream, "live");
  const campaign = projectCapture.resolveManagerProjectCapture({ message: "Start a content campaign named Fall launch by September 30, 2026", sourceMessageId: "campaign-message", sourceMessageCreatedAt: now, projects: [] });
  assert.equal(campaign.action.projectType, "content_campaign");
  assert.equal(campaign.action.name, "Fall launch");
  assert.equal(projectCapture.managerConversationProjectRecommendation(campaign.action).workstream, "content");
  const business = projectCapture.resolveManagerProjectCapture({ message: "Build a business project called New merch vendor for 2026-12-01", sourceMessageId: "business-message", sourceMessageCreatedAt: now, projects: [] });
  assert.equal(business.action.projectType, "business");
  assert.equal(projectCapture.managerConversationProjectRecommendation(business.action).workstream, "business");
});

test("Manager project capture refuses ambiguity, duplicates, secrets, questions, and implicit plans", () => {
  const input = (message, projects = []) => projectCapture.resolveManagerProjectCapture({ message, sourceMessageId: "message-project", sourceMessageCreatedAt: now, projects });
  assert.equal(input("We should release an EP this fall").status, "not_project");
  assert.equal(input("Create a project for 2026-10-15").status, "needs_clarification");
  assert.equal(input("Plan our EP release").status, "needs_clarification");
  assert.equal(input("Plan our EP release for October 2026").status, "needs_clarification");
  assert.equal(input("Plan our EP release for 2026-02-30").status, "needs_clarification");
  assert.equal(input("Create a release and tour project called Fall run for 2026-10-15").status, "needs_clarification");
  assert.equal(input("Plan our EP release and also a tour for 2026-10-15").status, "needs_clarification");
  assert.equal(input("Create a business project called Rotate API key: sk-secret-value for 2026-10-15").status, "blocked_sensitive");
  assert.equal(input("Should we plan our EP release for 2026-10-15?").status, "not_project");
  assert.equal(input("Plan our EP release for 2026-10-15?").status, "needs_clarification");
  const duplicate = input("Plan our ÉP release for 2026-10-15", [{ id: "project-a", type: "release", status: "active", name: "EP release", dueAt: new Date("2026-10-15T12:00:00.000Z") }]);
  assert.equal(duplicate.status, "duplicate");
  assert.equal(duplicate.duplicateProjectId, "project-a");
});

test("Manager event capture stages one exact timezone-safe event and unknown lineup review", () => {
  const members = [{ id: "member-b", name: "Jordan" }, { id: "member-a", name: "Alex" }];
  const captured = eventCapture.resolveManagerEventCapture({
    message: 'Record a confirmed gig called "Bluebird show" on October 15, 2026 at 7:00 PM at "Bluebird Theater"',
    sourceMessageId: "message-event",
  ...
  assert.equal(captured.status, "ready");
  assert.equal(captured.action.type, "create_conversation_event");
  assert.equal(captured.action.eventType, "gig");
  assert.equal(captured.action.status, "confirmed");
  assert.equal(captured.action.title, "Bluebird show");
  assert.equal(captured.action.startsAt, "2026-10-16T00:00:00.000Z");
  assert.equal(captured.action.timezone, "America/Chicago");
  assert.equal(captured.action.locationName, "Bluebird Theater");
  assert.deepEqual(captured.action.bandMemberIds, ["member-a", "member-b"]);
  assert.match(captured.preview, /Oct 15, 2026, 7:00 PM CDT/);
  assert.match(captured.preview, /2 active members will start as unknown/);
  assert.match(captured.preview, /does not contact anyone or add an external calendar event/i);
  assert.equal(eventCapture.managerConversationEventActionMatchesMessage(captured.action, { id: "message-event", content: 'Record a confirmed gig called "Bluebird show" on October 15, 2026 at 7:00 PM at "Bluebird Theater"', createdAt: now }, [], members), true);
  assert.equal(eventCapture.managerConversationEventActionMatchesMessage(captured.action, { id: "message-event", content: 'Record a confirmed gig called "Bluebird show" on October 15, 2026 at 7:00 PM at "Bluebird Theater"', createdAt: now }, [], members.slice(0, 1)), false);
  const recommendation = eventCapture.managerConversationEventRecommendation(captured.action);
  assert.equal(recommendation.proposedAction.type, "create_conversation_event");
  assert.equal(recommendation.workstream, "live");
  assert.deepEqual(recommendation.evidenceIds, ["member-a", "member-b"]);

  const rehearsal = eventCapture.resolveManagerEventCapture({ message: "Schedule a rehearsal called Album run-through on 2026-10-15 at 19:00", sourceMessageId: "message-rehearsal", sourceMessageCreatedAt: now, timezone: "America/Chicago", events: [], members: [] });
  assert.equal(rehearsal.action.eventType, "rehearsal");
  assert.equal(rehearsal.action.status, "draft");
  assert.equal(rehearsal.action.locationName, null);
});

test("Manager event capture refuses unsafe, ambiguous, duplicate, and timezone-invalid scheduling", () => {
  const input = (message, timezone = "America/Chicago", events = []) => eventCapture.resolveManagerEventCapture({ message, sourceMessageId: "message-event", sourceMessageCreatedAt: now, timezone, events, members: [{ id: "member-a", name: "Alex" }] });
  assert.equal(input("We should rehearse next week").status, "not_event");
  assert.equal(input("Schedule a rehearsal").status, "needs_clarification");
  assert.equal(input("Schedule a rehearsal called Album run-through on 2026-10-15 at 7:00 PM", null).status, "needs_clarification");
  assert.equal(input("Schedule a rehearsal called Album run-through on 2026-02-30 at 7:00 PM").status, "needs_clarification");
  assert.equal(input("Schedule a rehearsal called Album run-through on 2026-10-15 at 7").status, "needs_clarification");
  assert.equal(input("Schedule a rehearsal called Rotate API key: sk-secret-value on 2026-10-15 at 7:00 PM").status, "blocked_sensitive");
  assert.equal(input("Schedule a rehearsal called Album run-through on 2026-10-15 at 7:00 PM?").status, "needs_clarification");
  assert.equal(input("Schedule a rehearsal called First on 2026-10-15 at 7:00 PM and also a meeting called Second on 2026-10-16 at 7:00 PM").status, "needs_clarification");
  assert.equal(input("Schedule a rehearsal called Spring clock on 2026-03-08 at 2:30 AM").status, "needs_clarification");
  assert.equal(input("Schedule a rehearsal called Fall clock on 2026-11-01 at 1:30 AM").status, "needs_clarification");
  const duplicateStart = new Date("2026-10-16T00:00:00.000Z");
  const duplicate = input("Record a confirmed gig called Bluebird show on 2026-10-15 at 7:00 PM", "America/Chicago", [{ id: "event-a", type: "gig", status: "confirmed", title: "Bluebird Show", startsAt: duplicateStart }]);
  assert.equal(duplicate.status, "duplicate");
  assert.equal(duplicate.duplicateEventId, "event-a");
});

test("Manager event availability resolves one active member and current event into an exact reviewed change", () => {
  const event = { id: "event-a", title: "Bluebird show", status: "confirmed", startsAt: new Date("2026-10-16T00:00:00.000Z"), updatedAt: new Date("2026-07-12T11:00:00.000Z"), participants: [{ id: "participant-a", bandMemberId: "member-a", response: "unknown", respondedAt: null }] };
  const members = [{ id: "member-a", name: "Morgan Lee" }, { id: "member-b", name: "Jordan Smith" }];
  const input = (message, events = [event], activeMembers = members) => eventAvailability.resolveManagerEventAvailability({ message, sourceMessageId: "message-availability", sourceMessageCreatedAt: now, events, members: activeMembers });
  const unavailable = input('Morgan can\'t make "Bluebird show"');
  assert.equal(unavailable.status, "ready");
  assert.equal(unavailable.action.type, "update_conversation_event_availability");
  assert.equal(unavailable.action.eventId, event.id);
  assert.equal(unavailable.action.eventUpdatedAt, event.updatedAt.toISOString());
  assert.equal(unavailable.action.bandMemberId, "member-a");
  assert.equal(unavailable.action.participantId, "participant-a");
  assert.equal(unavailable.action.previousResponse, "unknown");
  assert.equal(unavailable.action.response, "unavailable");
  assert.match(unavailable.preview, /Unknown → Unavailable/);
  assert.match(unavailable.preview, /does not notify the member or save a private explanation/i);
  assert.equal(eventAvailability.managerConversationEventAvailabilityActionMatchesMessage(unavailable.action, { id: "message-availability", content: 'Morgan can\'t make "Bluebird show"', createdAt: now }, [event], members), true);
  assert.equal(eventAvailability.managerConversationEventAvailabilityActionMatchesMessage(unavailable.action, { id: "message-availability", content: 'Morgan can\'t make "Bluebird show"', createdAt: now }, [{ ...event, participants: [{ ...event.participants[0], response: "tentative", respondedAt: now }] }], members), false);
  const recommendation = eventAvailability.managerConversationEventAvailabilityRecommendation(unavailable.action);
  assert.equal(recommendation.workstream, "live");
  assert.equal(recommendation.priority, "med");
  assert.deepEqual(recommendation.evidenceIds, ["event-a", "member-a", "participant-a"]);

  const available = input('Mark Jordan available for "Bluebird show"');
  assert.equal(available.action.previousResponse, "unknown");
  assert.equal(available.action.participantId, null);
  assert.equal(available.action.response, "available");
  const tentative = input('Set Jordan\'s availability for "Bluebird show" to tentative');
  assert.equal(tentative.action.response, "tentative");
  const statement = input('Jordan is unavailable for "Bluebird show"');
  assert.equal(statement.action.response, "unavailable");
});

test("Manager event availability refuses questions, secrets, ambiguity, multiple people, stale events, and no-op responses", () => {
  const event = { id: "event-a", title: "Bluebird show", status: "confirmed", startsAt: new Date("2026-10-16T00:00:00.000Z"), updatedAt: new Date("2026-07-12T11:00:00.000Z"), participants: [{ id: "participant-a", bandMemberId: "member-a", response: "available", respondedAt: new Date("2026-07-12T10:00:00.000Z") }] };
  const members = [{ id: "member-a", name: "Morgan Lee" }, { id: "member-b", name: "Morgan Smith" }];
  const input = (message, events = [event], activeMembers = members) => eventAvailability.resolveManagerEventAvailability({ message, sourceMessageId: "message-availability", sourceMessageCreatedAt: now, events, members: activeMembers });
  assert.equal(input('Can Morgan Lee make "Bluebird show"?').status, "not_availability");
  assert.equal(input('Morgan Lee is unavailable for "Bluebird show"?').status, "not_availability");
  assert.equal(input('Mark Morgan Lee available for "Bluebird show"').status, "already_current");
  assert.equal(input('Mark Morgan unavailable for "Bluebird show"').status, "needs_clarification");
  assert.equal(input('Mark Morgan Lee and Jordan Smith available for "Bluebird show"').status, "needs_clarification");
  assert.equal(input('Mark Morgan Lee available for "Bluebird show" and also another event').status, "needs_clarification");
  assert.equal(input('Record Morgan Lee available for "Bluebird show" with API key: sk-secret-value').status, "blocked_sensitive");
  assert.equal(input('Mark Morgan Lee unavailable for "Missing show"').status, "needs_clarification");
  assert.equal(input('Mark Morgan Lee unavailable for "Bluebird show"', [{ ...event, status: "completed" }]).status, "needs_clarification");
  assert.equal(input('Mark Morgan Lee unavailable for "Bluebird show"', [event, { ...event, id: "event-b", updatedAt: new Date("2026-07-12T11:01:00.000Z"), participants: [] }]).status, "needs_clarification");
  assert.equal(input('We should check everyone\'s availability').status, "not_availability");
  assert.equal(eventAvailability.managerMessageIsEventAvailabilityIntent('Mark Morgan available for "Bluebird show"'), true);
  assert.equal(eventAvailability.managerMessageIsEventAvailabilityIntent('Mark "Check member availability" done'), false);
});

test("Manager task updates resolve one current task and stage exact reviewed changes", () => {
  const task = { id: "task-a", title: "Confirm rehearsal", status: "todo", dueAt: new Date("2026-07-18T12:00:00.000Z"), updatedAt: new Date("2026-07-12T11:00:00.000Z"), blockedReason: null, waitingOn: null, prerequisites: [], dependents: [] };
  const input = (message, tasks = [task], timezone = "America/Chicago") => taskUpdate.resolveManagerTaskUpdate({ message, sourceMessageId: "message-a", sourceMessageCreatedAt: now, timezone, tasks });
  const complete = input('Can you mark "Confirm rehearsal" done?');
  assert.equal(complete.status, "ready");
  assert.equal(complete.action.type, "update_conversation_task");
  assert.equal(complete.action.taskId, task.id);
  assert.equal(complete.action.operation, "complete");
  assert.match(complete.preview, /Task: Confirm rehearsal/);
  assert.match(complete.preview, /Change: Mark done/);
  assert.deepEqual(taskUpdate.managerConversationTaskUpdateRecommendation(complete.action).evidenceIds, [task.id]);
  assert.equal(taskUpdate.managerConversationTaskUpdateActionMatchesMessage(complete.action, { id: "message-a", content: 'Can you mark "Confirm rehearsal" done?', createdAt: now }, [task]), true);
  assert.equal(taskUpdate.managerConversationTaskUpdateActionMatchesMessage(complete.action, { id: "message-a", content: 'Can you mark "Confirm rehearsal" done?', createdAt: now }, [{ ...task, updatedAt: new Date("2026-07-12T11:01:00.000Z") }]), false);

  const blocked = input("Block Confirm rehearsal because the promoter has not confirmed the room");
  assert.equal(blocked.action.operation, "block");
  assert.equal(blocked.action.blockedReason, "the promoter has not confirmed the room");
  const waiting = input("Set Confirm rehearsal waiting on the promoter");
  assert.equal(waiting.action.operation, "set_waiting_on");
  assert.equal(waiting.action.waitingOn, "the promoter");
  const moved = input("Move Confirm rehearsal to tomorrow");
  assert.equal(moved.action.operation, "reschedule");
  assert.equal(moved.action.dueDate, "2026-07-13");
  assert.equal(moved.action.dateBasisTimezone, "America/Chicago");
  assert.equal(taskUpdate.managerConversationTaskUpdateDueAt(moved.action).toISOString(), "2026-07-13T12:00:00.000Z");
  assert.equal(input("Clear the due date for Confirm rehearsal").action.operation, "clear_due_date");
});

test("Manager task updates clarify ambiguity, no-ops, unsafe text, and task constraints", () => {
  const base = { id: "task-a", title: "Confirm rehearsal", status: "todo", dueAt: new Date("2026-07-18T12:00:00.000Z"), updatedAt: new Date("2026-07-12T11:00:00.000Z"), blockedReason: null, waitingOn: null, prerequisites: [], dependents: [] };
  const input = (message, tasks = [base], timezone = "America/Chicago") => taskUpdate.resolveManagerTaskUpdate({ message, sourceMessageId: "message-a", sourceMessageCreatedAt: now, timezone, tasks });
  assert.equal(input("We should finish Confirm rehearsal").status, "not_update");
  assert.equal(input("Mark it done").status, "needs_clarification");
  assert.equal(input("Mark soundcheck done", [{ ...base, id: "task-a", title: "Confirm soundcheck" }, { ...base, id: "task-b", title: "Schedule soundcheck" }]).status, "needs_clarification");
  assert.equal(input("Block Confirm rehearsal because API key: sk-secret-value").status, "blocked_sensitive");
  assert.equal(input("Move Confirm rehearsal to next Friday").status, "needs_clarification");
  assert.equal(input("Move Confirm rehearsal to tomorrow", [base], null).status, "needs_clarification");
  assert.equal(input("Move Confirm rehearsal to 2026-07-18").status, "already_current");
  assert.equal(input("Clear the waiting status for Confirm rehearsal").status, "already_current");
  assert.equal(input("Mark Confirm rehearsal done", [{ ...base, prerequisites: [{ prerequisiteTask: { id: "task-prereq", title: "Book the room", status: "todo", dueAt: new Date("2026-07-17T12:00:00.000Z") } }] }]).status, "needs_clarification");
  assert.equal(input("Move Confirm rehearsal to 2026-07-16", [{ ...base, prerequisites: [{ prerequisiteTask: { id: "task-prereq", title: "Book the room", status: "done", dueAt: new Date("2026-07-17T12:00:00.000Z") } }] }]).status, "needs_clarification");
  assert.equal(input("Start Confirm rehearsal", [{ ...base, status: "done" }]).status, "needs_clarification");
});

test("Manager conversational assignment stages one exact task and active member with capacity context", () => {
  const task = { id: "task-a", title: "Confirm rehearsal", status: "todo", updatedAt: new Date("2026-07-12T11:00:00.000Z"), bandMemberId: null, ownerLabel: null };
  const members = [{ id: "member-a", name: "Alex Rivera", checkInId: "checkin-a", availability: "available" }, { id: "member-b", name: "Morgan Lee", checkInId: "checkin-b", availability: "limited" }];
  const input = (message, tasks = [task], availableMembers = members) => taskAssignment.resolveManagerTaskAssignment({ message, sourceMessageId: "message-a", sourceMessageCreatedAt: now, tasks, members: availableMembers });
  const assigned = input('Assign "Confirm rehearsal" to Morgan');
  assert.equal(assigned.status, "ready");
  assert.equal(assigned.action.type, "assign_conversation_task");
  assert.equal(assigned.action.taskId, task.id);
  assert.equal(assigned.action.bandMemberId, "member-b");
  assert.equal(assigned.action.availability, "limited");
  assert.match(assigned.preview, /Owner: Unassigned → Morgan Lee/);
  assert.match(assigned.preview, /Availability: Limited/);
  assert.deepEqual(taskAssignment.managerConversationTaskAssignmentRecommendation(assigned.action).evidenceIds, [task.id, "member-b", "checkin-b"]);
  assert.equal(taskAssignment.managerConversationTaskAssignmentActionMatchesMessage(assigned.action, { id: "message-a", content: 'Assign "Confirm rehearsal" to Morgan', createdAt: now }, [task], members), true);
  assert.equal(taskAssignment.managerConversationTaskAssignmentActionMatchesMessage(assigned.action, { id: "message-a", content: 'Assign "Confirm rehearsal" to Morgan', createdAt: now }, [{ ...task, updatedAt: new Date("2026-07-12T11:01:00.000Z") }], members), false);

  const reassigned = input("Make Morgan Lee responsible for Confirm rehearsal", [{ ...task, bandMemberId: "member-a", ownerLabel: "Alex Rivera" }]);
  assert.equal(reassigned.status, "ready");
  assert.match(reassigned.preview, /Alex Rivera → Morgan Lee/);
  assert.equal(input("Give Confirm rehearsal to Alex Rivera").action.bandMemberId, "member-a");
});

test("Manager conversational assignment clarifies ambiguity, unavailable people, completed work, and no-ops", () => {
  const task = { id: "task-a", title: "Confirm rehearsal", status: "todo", updatedAt: new Date("2026-07-12T11:00:00.000Z"), bandMemberId: null, ownerLabel: null };
  const input = (message, tasks, members) => taskAssignment.resolveManagerTaskAssignment({ message, sourceMessageId: "message-a", sourceMessageCreatedAt: now, tasks: tasks ?? [task], members: members ?? [{ id: "member-a", name: "Alex", checkInId: null, availability: "unknown" }] });
  assert.equal(input("We should give Confirm rehearsal to Alex").status, "not_assignment");
  assert.equal(input("Assign it to Alex").status, "needs_clarification");
  assert.equal(input("Assign Confirm rehearsal to Alex and also update the setlist").status, "needs_clarification");
  assert.equal(input("Assign soundcheck to Alex", [{ ...task, id: "task-a", title: "Confirm soundcheck" }, { ...task, id: "task-b", title: "Schedule soundcheck" }]).status, "needs_clarification");
  assert.equal(input("Assign Confirm rehearsal to Alex", undefined, [{ id: "member-a", name: "Alex Rivera", checkInId: null, availability: "unknown" }, { id: "member-b", name: "Alex Morgan", checkInId: null, availability: "unknown" }]).status, "needs_clarification");
  assert.equal(input("Assign Confirm rehearsal to Morgan", undefined, [{ id: "member-b", name: "Morgan", checkInId: "checkin-b", availability: "unavailable" }]).status, "blocked_unavailable");
  assert.equal(input("Assign Confirm rehearsal to Alex", [{ ...task, status: "done" }]).status, "needs_clarification");
  assert.equal(input("Assign Confirm rehearsal to Alex", [{ ...task, bandMemberId: "member-a", ownerLabel: "Alex" }]).status, "already_current");
  assert.equal(input("Assign Confirm rehearsal to Former Member", undefined, []).status, "needs_clarification");
});
...
test("manager action authorization is code-owned and defaults to forbidden", () => {
  ...
  assert.equal(policy.classifyManagerAction("update_conversation_task"), "internal");
  assert.equal(policy.classifyManagerAction("assign_conversation_task"), "internal");
  assert.equal(policy.classifyManagerAction("create_conversation_project"), "internal");
  assert.equal(policy.classifyManagerAction("create_conversation_event"), "internal");
  assert.equal(policy.classifyManagerAction("update_conversation_event_availability"), "internal");
...
test("accepted conversational project is source-bound, atomic, duplicate-safe, tenant-scoped, linked, and audited", async () => {
  const source = { id: "message-project", content: 'Create a release project called "Midnight Signals" due 2026-10-15', createdAt: new Date("2026-07-12T12:00:00.000Z") };
  const captured = projectCapture.resolveManagerProjectCapture({ message: source.content, sourceMessageId: source.id, sourceMessageCreatedAt: source.createdAt, projects: [] });
  assert.equal(captured.status, "ready");
  let outcome = "suggested"; let projects = []; let milestoneCount = 0; const audits = [];
  const client = {
    managerRecommendation: {
      findFirst: async ({ where }) => where.managerRun.artistId === "artist-a" ? { id: "rec-project", outcome, taskId: null, decisionId: null, memoryFactId: null, projectId: null, task: null, decision: null, memoryFact: null, project: null, proposedAction: captured.action, evidence: [], managerRun: { message: { id: "answer-project", conversationId: "conversation-a", createdAt: new Date("2026-07-12T12:00:01.000Z") } } } : null,
      updateMany: async ({ data }) => { if (outcome !== "suggested") return { count: 0 }; outcome = data.outcome; return { count: 1 }; },
      update: async ({ data }) => ({ id: "rec-project", outcome, taskId: null, decisionId: null, memoryFactId: null, projectId: data.projectId ?? null })
    },
    managerMessage: { findFirst: async ({ where }) => where.id === source.id && where.conversationId === "conversation-a" ? source : null },
    artistProject: {
      findMany: async ({ where }) => projects.filter((project) => project.artistId === where.artistId),
      create: async ({ data }) => { const project = { id: "project-chat", ...data }; projects = [project]; return project; }
    },
    task: {
      createMany: async ({ data }) => { milestoneCount += data.length; assert.equal(data.every((task) => task.artistId === "artist-a" && task.projectId === "project-chat" && task.sourceKey.startsWith("project_plan_v1:project-chat:")), true); return { count: data.length }; }
    }
  };
  client.$transaction = async (fn) => fn(client);
  const service = new managerMod.ManagerService({ client }, { log: async (entry) => audits.push(entry) }, { get: () => false });
  const accepted = await service.recommendation("artist-a", "rec-project", "accepted", {}, "member@test", "operator-a");
  assert.equal(accepted.outcome, "completed");
  assert.equal(accepted.projectId, "project-chat");
  assert.equal(projects[0].type, "release");
  assert.equal(projects[0].status, "active");
  assert.equal(projects[0].name, "Midnight Signals");
  assert.equal(projects[0].dueAt.toISOString(), "2026-10-15T12:00:00.000Z");
  assert.equal(milestoneCount, 6);
  const projectAudit = audits.find((entry) => entry.action === "project.created_from_manager_chat");
  assert.equal(projectAudit.aggregateId, "project-chat");
  assert.equal(projectAudit.metadata.createdMilestoneCount, 6);
  assert.equal(projectAudit.metadata.sourceMessageId, source.id);
  assert.equal(JSON.stringify(projectAudit.metadata).includes(source.content), false);
  await assert.rejects(() => service.recommendation("artist-a", "rec-project", "accepted", {}, "member@test", "operator-a"), /already been decided/);
  await assert.rejects(() => service.recommendation("artist-b", "rec-project", "accepted", {}, "member@test", "operator-b"), (error) => error?.getStatus?.() === 404);
  assert.equal(projects.length, 1);
  assert.equal(milestoneCount, 6);
});

test("accepted conversational event is source-bound, timezone-safe, lineup-bound, tenant-scoped, linked, and audited", async () => {
  const source = { id: "message-event", content: 'Record a confirmed gig called "Bluebird show" on 2026-10-15 at 7:00 PM at "Bluebird Theater"', createdAt: new Date("2026-07-12T12:00:00.000Z") };
  const activeMembers = [{ id: "member-a", name: "Alex" }, { id: "member-b", name: "Jordan" }];
  const captured = eventCapture.resolveManagerEventCapture({ message: source.content, sourceMessageId: source.id, sourceMessageCreatedAt: source.createdAt, timezone: "America/Chicago", events: [], members: activeMembers });
  assert.equal(captured.status, "ready");
  let outcome = "suggested"; let events = []; let participantCount = 0; const audits = [];
  const client = {
    managerRecommendation: {
      findFirst: async ({ where }) => where.managerRun.artistId === "artist-a" ? { id: "rec-event", outcome, taskId: null, decisionId: null, memoryFactId: null, projectId: null, eventId: null, task: null, decision: null, memoryFact: null, project: null, event: null, proposedAction: captured.action, evidence: captured.action.bandMemberIds, managerRun: { message: { id: "answer-event", conversationId: "conversation-a", createdAt: new Date("2026-07-12T12:00:01.000Z") } } } : null,
      updateMany: async ({ data }) => { if (outcome !== "suggested") return { count: 0 }; outcome = data.outcome; return { count: 1 }; },
      update: async ({ data }) => ({ id: "rec-event", outcome, taskId: null, decisionId: null, memoryFactId: null, projectId: null, eventId: data.eventId ?? null })
    },
    managerMessage: { findFirst: async ({ where }) => where.id === source.id && where.conversationId === "conversation-a" ? source : null },
    bandMember: { findMany: async ({ where }) => where.artistId === "artist-a" && where.active ? activeMembers : [] },
    bandEvent: {
      findMany: async ({ where }) => events.filter((event) => event.artistId === where.artistId),
      create: async ({ data }) => { const event = { id: "event-chat", ...data }; events = [event]; return event; }
    },
    eventParticipant: {
      createMany: async ({ data }) => { participantCount += data.length; assert.deepEqual(data.map((participant) => participant.bandMemberId), ["member-a", "member-b"]); assert.equal(data.every((participant) => participant.eventId === "event-chat" && participant.response === "unknown"), true); return { count: data.length }; }
    }
  };
  client.$transaction = async (fn) => fn(client);
  const service = new managerMod.ManagerService({ client }, { log: async (entry) => audits.push(entry) }, { get: () => false });
  const accepted = await service.recommendation("artist-a", "rec-event", "accepted", {}, "member@test", "operator-a");
  assert.equal(accepted.outcome, "completed");
  assert.equal(accepted.eventId, "event-chat");
  assert.equal(events[0].artistId, "artist-a");
  assert.equal(events[0].type, "gig");
  assert.equal(events[0].status, "confirmed");
  assert.equal(events[0].title, "Bluebird show");
  assert.equal(events[0].startsAt.toISOString(), "2026-10-16T00:00:00.000Z");
  assert.equal(events[0].timezone, "America/Chicago");
  assert.equal(events[0].locationName, "Bluebird Theater");
  assert.equal(participantCount, 2);
  const eventAudit = audits.find((entry) => entry.action === "event.created_from_manager_chat");
  assert.equal(eventAudit.aggregateId, "event-chat");
  assert.equal(eventAudit.metadata.participantCount, 2);
  assert.equal(eventAudit.metadata.locationPresent, true);
  assert.equal(JSON.stringify(eventAudit.metadata).includes(source.content), false);
  await assert.rejects(() => service.recommendation("artist-a", "rec-event", "accepted", {}, "member@test", "operator-a"), /already been decided/);
  await assert.rejects(() => service.recommendation("artist-b", "rec-event", "accepted", {}, "member@test", "operator-b"), (error) => error?.getStatus?.() === 404);
  assert.equal(events.length, 1);
  assert.equal(participantCount, 2);
});

test("accepted conversational event availability is source-bound, optimistic, tenant-safe, linked, and audited", async () => {
  const source = { id: "message-availability", content: 'Morgan can\'t make "Bluebird show"', createdAt: new Date("2026-07-12T12:00:00.000Z") };
  const member = { id: "member-a", name: "Morgan Lee" };
  let participant = { id: "participant-a", eventId: "event-a", bandMemberId: member.id, response: "unknown", respondedAt: null };
  const event = { id: "event-a", artistId: "artist-a", title: "Bluebird show", status: "confirmed", startsAt: new Date("2026-10-16T00:00:00.000Z"), updatedAt: new Date("2026-07-12T11:00:00.000Z"), participants: [participant] };
  const captured = eventAvailability.resolveManagerEventAvailability({ message: source.content, sourceMessageId: source.id, sourceMessageCreatedAt: source.createdAt, events: [event], members: [member] });
  assert.equal(captured.status, "ready");
  let outcome = "suggested"; let writes = 0; const audits = [];
  const client = {
    managerRecommendation: {
      findFirst: async ({ where }) => where.managerRun.artistId === "artist-a" ? { id: "rec-availability", outcome, taskId: null, decisionId: null, memoryFactId: null, projectId: null, eventId: null, task: null, decision: null, memoryFact: null, project: null, event: null, proposedAction: captured.action, evidence: [event.id, member.id, participant.id], managerRun: { message: { id: "answer-availability", conversationId: "conversation-a", createdAt: new Date("2026-07-12T12:00:01.000Z") } } } : null,
      updateMany: async ({ data }) => { if (outcome !== "suggested") return { count: 0 }; outcome = data.outcome; return { count: 1 }; },
      update: async ({ data }) => ({ id: "rec-availability", outcome, taskId: null, decisionId: null, memoryFactId: null, projectId: null, eventId: data.eventId ?? null })
    },
    managerMessage: { findFirst: async ({ where }) => where.id === source.id && where.conversationId === "conversation-a" ? source : null },
    bandMember: { findMany: async ({ where }) => where.artistId === "artist-a" && where.active ? [member] : [] },
    bandEvent: { findMany: async ({ where }) => where.artistId === "artist-a" ? [{ ...event, participants: [participant] }] : [] },
    eventParticipant: {
      updateMany: async ({ where, data }) => {
        if (where.id !== participant.id || where.eventId !== event.id || where.bandMemberId !== member.id || where.response !== participant.response || where.respondedAt !== participant.respondedAt) return { count: 0 };
        writes += 1;
  ...
  participant = { ...participant, response: "tentative", respondedAt: new Date("2026-07-12T11:30:00.000Z") };
  await assert.rejects(() => service.recommendation("artist-a", "rec-availability", "accepted", {}, "member@test", "operator-a"), /no longer matches|changed/i);
  assert.equal(outcome, "suggested");
  assert.equal(writes, 0);
  participant = { ...participant, response: "unknown", respondedAt: null };

  const accepted = await service.recommendation("artist-a", "rec-availability", "accepted", {}, "member@test", "operator-a");
  assert.equal(accepted.outcome, "completed");
  assert.equal(accepted.eventId, event.id);
  assert.equal(participant.response, "unavailable");
  assert.ok(participant.respondedAt instanceof Date);
  assert.equal(writes, 1);
  const availabilityAudit = audits.find((entry) => entry.action === "event.availability_recorded_from_manager_chat");
  assert.equal(availabilityAudit.aggregateId, participant.id);
  assert.deepEqual(availabilityAudit.metadata, { recommendationId: "rec-availability", sourceMessageId: source.id, policyVersion: "manager_event_availability_v1", eventId: event.id, bandMemberId: member.id, previousResponse: "unknown", response: "unavailable", participantExisted: true });
  assert.equal(JSON.stringify(availabilityAudit.metadata).includes(source.content), false);
  await assert.rejects(() => service.recommendation("artist-a", "rec-availability", "accepted", {}, "member@test", "operator-a"), /already been decided/);
  await assert.rejects(() => service.recommendation("artist-b", "rec-availability", "accepted", {}, "member@test", "operator-b"), (error) => error?.getStatus?.() === 404);
  assert.equal(writes, 1);
});

test("accepted conversational task update is source-bound, optimistic, tenant-safe, atomic, and audited", async () => {
  const source = { id: "message-update", content: 'Mark "Confirm rehearsal" done', createdAt: new Date("2026-07-12T12:00:00.000Z") };
  ...
  const client = {
    ...
    task: {
      ...
      updateMany: async ({ where, data }) => {
        ...
        taskWrites += 1;
        task = { ...task, ...data, updatedAt: new Date("2026-07-12T12:01:00.000Z") };
        return { count: 1 };
      },
  ...
  task = { ...task, updatedAt: originalUpdatedAt };

  const accepted = await service.recommendation("artist-a", "rec-update", "accepted", {}, "member@test", "operator-a");
  assert.equal(accepted.outcome, "completed");
  ...
  assert.equal(JSON.stringify(taskAudit.metadata).includes(source.content), false);
  await assert.rejects(() => service.recommendation("artist-b", "rec-update", "accepted", {}, "member@test", "operator-b"), (error) => error?.getStatus?.() === 404);
  await assert.rejects(() => service.recommendation("artist-a", "rec-update", "accepted", {}, "member@test", "operator-a"), /already been decided/);
  assert.equal(taskWrites, 1);
});

test("accepted conversational assignment is source-bound, capacity-aware, optimistic, tenant-safe, and audited", async () => {
  ...
  assert.equal(captured.status, "ready");
  let outcome = "suggested"; let taskWrites = 0; const audits = [];
  const client = {
    managerRecommendation: {
      findFirst: async ({ where }) => where.managerRun.artistId === "artist-a" ? { id: "rec-owner", outcome, taskId: null, decisionId: null, memoryFactId: null, task: null, decision: null, memoryFact: null, proposedAction: captured.action, evidence: [task.id, member.id, originalCheckIn.id], managerRun: { message: { id: "answer-owner", conversationId: "conversation-a", createdAt: new Date("2026-07-12T12:00:01.000Z") } } } : null,
      updateMany: async ({ where, data }) => { if (where.id !== "rec-owner" || outcome !== "suggested") return { count: 0 }; outcome = data.outcome; return { count: 1 }; },
      update: async ({ data }) => ({ id: "rec-owner", outcome, taskId: data.taskId ?? null, decisionId: null, memoryFactId: null })
    },
    managerMessage: { findFirst: async ({ where }) => where.id === source.id && where.conversationId === "conversation-a" ? source : null },
    task: {
      findMany: async () => [task],
      findFirst: async ({ where }) => where.id === task.id && where.artistId === task.artistId ? task : null,
      updateMany: async ({ where, data }) => {
        if (where.id !== task.id || where.artistId !== task.artistId || where.updatedAt.getTime() !== task.updatedAt.getTime() || where.bandMemberId !== task.bandMemberId || where.ownerLabel !== task.ownerLabel) return { count: 0 };
        taskWrites += 1;
        task = { ...task, ...data, updatedAt: new Date("2026-07-12T12:01:00.000Z") };
        return { count: 1 };
      }
    ...
    bandMember: {
      ...
      findFirst: async ({ where }) => where.id === member.id && where.artistId === member.artistId && member.active ? member : null
    }
  };
  client.$transaction = async (fn) => fn(client);
  const service = new managerMod.ManagerService({ client }, { log: async (entry) => audits.push(entry) }, { get: () => false });

  member = { ...member, checkIns: [{ ...originalCheckIn, id: "checkin-new", status: "unavailable", createdAt: new Date("2026-07-12T11:30:00.000Z") }] };
  await assert.rejects(() => service.recommendation("artist-a", "rec-owner", "accepted", {}, "member@test", "operator-a"), /no longer matches|unavailable|check-in/i);
  assert.equal(outcome, "suggested");
  assert.equal(taskWrites, 0);
  member = { ...member, checkIns: [originalCheckIn] };

  const accepted = await service.recommendation("artist-a", "rec-owner", "accepted", {}, "member@test", "operator-a");
  assert.equal(accepted.outcome, "completed");
  assert.equal(accepted.taskId, task.id);
  assert.equal(task.bandMemberId, member.id);
  assert.equal(task.ownerLabel, member.name);
  assert.equal(taskWrites, 1);
  const taskAudit = audits.find((entry) => entry.action === "task.assigned_from_manager_chat");
  assert.equal(taskAudit.aggregateId, task.id);
  assert.equal(taskAudit.metadata.checkInId, originalCheckIn.id);
  assert.equal(JSON.stringify(taskAudit.metadata).includes(originalCheckIn.note), false);
  await assert.rejects(() => service.recommendation("artist-b", "rec-owner", "accepted", {}, "member@test", "operator-b"), (error) => error?.getStatus?.() === 404);
  await assert.rejects(() => service.recommendation("artist-a", "rec-owner", "accepted", {}, "member@test", "operator-a"), /already been decided/);
  assert.equal(taskWrites, 1);
});
...
test("manager feedback and memory correction payloads are strict", () => {
  ...
  assert.equal(managerSchemas.managerResponseEvalResolutionSchema.safeParse({ candidateVersion: "manager_os_v31", note: "Reviewed the corrected behavior against this case." }).success, true);
    ...
    managerRecommendation: {
      findMany: async (input) => {
        ...
        return [{
          ...
          managerRun: { promptVersion: "manager_os_v31", cadence: "daily" },
    ...
    managerMessage: {
      findMany: async ({ where }) => {
        ...
        if (where.role === "assistant") return [{
          ...
          managerRun: { promptVersion: "manager_os_v31", mode: "deterministic" },
...
test("Vault import is the default song path and Manager stays honest after import", async () => {
  ...
  assert.deepEqual(liveShape.songs.map((song) => song.title), ["Harbor Lights", "Sidewalk Radio", "Everyday"]);
  assert.equal(liveShape.setlists.length, 1);
  assert.ok(!liveShape.songs.some((song) => /parked demo|trailer|booking calendar|cover example/i.test(song.title)));
  const emptyPublished = catalogImport.planCatalogImport({
    vault: {
      schema_version: 3,
      songs: [
  ...
  assert.equal(emptyPublished.songs.length, 0);
  assert.equal(emptyPublished.setlists.length, 0);
  const emptyLive = managerFacts({
    ...
    events: [],
    prospects: [],
    ...catalogImport.managerRecordsFromCatalogPlan(emptyPublished)
  });
  ...
  assert.match(emptySetlist.answer, /setlist_ready_default_import|default_live/i);
  ...
  assert.match(writer.answer, /jeff story/i);
  assert.match(writer.answer, /default live|default-live|setlist_ready_default_import/i);
  assert.match(writer.answer, /not a fourth live band/i);
  assert.doesNotMatch(writer.answer, /writer project|harbor lights|sidewalk radio/i);
  const publishedSetlist = intelligence.deterministicManagerChat(managerFacts({
    artist: { id: "artist-a", name: "My Artist" },
    profile: { id: "profile-a", intakeCompletedAt: null, decisionStyle: "guided", twelveMonthAmbition: null },
    members: [],
    goals: [],
...
test("manager grounding rejects a whole response with invented evidence", () => {
  ...
  const providerAvailabilityItem = { stableKey: "availability-event-a", title: "Mark Alex available", reason: "Provider suggestion", nextAction: "Update it", workstream: "live", priority: "low", evidenceIds: ["event-a", "member-a"], proposedAction: { type: "update_conversation_event_availability", sourceMessageId: "message-a", sourceMessageCreatedAt: now.toISOString(), eventId: "event-a", eventUpdatedAt: now.toISOString(), eventTitle: "Saturday show", eventStartsAt: "2026-07-18T01:00:00.000Z", bandMemberId: "member-a", bandMemberName: "Alex", participantId: null, previousResponse: "unknown", previousRespondedAt: null, response: "available" } };
  assert.equal(service.chatOutputIsGrounded({ answer: "Update availability", citations: ["event-a", "member-a"], recommendation: providerAvailabilityItem }, actionFacts), false);
