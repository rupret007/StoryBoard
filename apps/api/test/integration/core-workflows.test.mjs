...
test("database integration: manager intake, confirmed gig, payment, and settlement remain tenant-scoped and audited", async () => {
  ...
  const blockedEvaluation = await manager.runEvaluation(artist.id, "manager_os_v28", operator.email, operator.id);
  ...
  const passingEvaluation = await manager.runEvaluation(artist.id, "manager_os_v28", operator.email, operator.id);
  ...
  assert.equal((await manager.runEvaluation(artist.id, "manager_os_v28", operator.email, operator.id)).passed, false);
  await assert.rejects(() => manager.resolveResponseEvalExample(artist.id, responseEval.id, { candidateVersion: "manager_os_v28", note: "The response behavior has been corrected." }, operator.email, operator.id), /same Manager version/);
  ...
  const responseEvaluation = await manager.runEvaluation(artist.id, "manager_os_v28", operator.email, operator.id);
  ...
  const actionRun = await client.managerRun.create({ data: { artistId: artist.id, cadence: "daily", mode: "deterministic", promptVersion: "manager_os_v28", inputFacts: {}, output: {}, trace: {} } });
