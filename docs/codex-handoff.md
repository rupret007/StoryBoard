...
| Cross-functional Manager intake, goals, memory, briefs, chat, recommendations | Done; executable idempotent 90-day starter plans, persistent multi-turn conversation, intent-aware fallback, strict evidence rejection, exact-response helpful/correction feedback, natural-response guardrails, evidence-backed 90-day post-show/outcome review, reviewed recommendation learning/cooldowns, correctable memory, append-only goal progress, owner/timeline-aware plan health, and owner-run offline eval gate |
...
2. **Release workflow:** Use real response feedback to design a reviewed response-fixture promotion path for a future candidate; do not copy raw conversations or auto-activate a version. `manager_os_v4` is the current code-registered version.
