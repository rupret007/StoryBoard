...
| Events, availability, show advance, songs/setlists, projects | Done; structured operations workspace, actionable gig editor, phone day-of view with editable custom run-of-show checkpoints, practical ordered setlist/song editing, shared `setlist_summary_v1` timing truth, `ops_next_action_v1` next-step links on existing event/setlist/money cards, evidence-backed show-readiness scoring, event-bound Calendar/Drive state with explicit approval preparation, and dry-run-default Vault/Show Night catalog import (`pnpm catalog:import` / `POST /songs/import`) onto the current artist only. Default seed dry-runs the checked-in `app_api.json` shape and still leaves the song table empty unless `VAULT_CATALOG_APPLY=true`; `SEED_DEMO_OPS` is optional generic practice data, not a live catalog. Parked PR #21 (dashboard show/set/booking feel) stays untouched. |
