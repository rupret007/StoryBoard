...
| Cross-functional Manager intake, goals, memory, briefs, chat, recommendations | Done; executable idempotent 90-day starter plans, persistent multi-turn conversation, intent-aware fallback, strict evidence rejection, editable structured band/lineup context with transparent coverage questions, global explainable pressure ranking before response limits, code-owned profile-over-memory source precedence and freshness/conflict assessment, evidence-reconciled goal measurements with explicit member sync, explicit review-before-save conversational memory, evidence-ranked blocked/deferred/waiting follow-through, owner-opted timezone-aware daily/weekly briefs with deterministic-default delivery, exact-response helpful/correction feedback, natural-response guardrails, evidence-backed 90-day post-show/outcome review, scheduled option→choice→expected-result→immutable-review decisions, reviewed recommendation and exact-answer learning/cooldowns, sensitivity-enforced provider context, correctable non-profile memory, append-only goal progress, owner/timeline-aware plan health, and owner-run offline eval gate |
...
| Tests | Compiled `node:test` (89 API cases) plus opt-in Postgres integration coverage for tenant links, idempotent Manager intake/90-day plans, authoritative goal measurement/reconciliation/replay, explicit conversational-memory confirmation/replay/isolation, timezone/period scheduling, atomic brief notifications and replay prevention, operating-context coverage/updates, profile-memory synchronization/source precedence, knowledge freshness/conflict projection, global priority-trace persistence and audited-fact cache freshness, task blocker/deferral/concurrency/isolation, memory correction and provider redaction, goal progress/health, exact response→run feedback, owner-reviewed response promotion/isolation/release blocking, decision proposal/framing/concurrency/tenant isolation/immutable review, outcome review/isolation, feedback upsert/isolation, recommendation cooldowns/eval promotion and pass/fail runs, multi-turn isolation, tenant-safe/idempotent Manager show-advance and project-plan execution, confirmed-event idempotency, merged event schedule validation, day-of isolation/current-next derivation, project milestone idempotency/isolation/readiness, availability/advance, prospect/campaign delivery, payment replay, currency-safe settlement math/PDF snapshots, roles, Telegram binding, and audits. Chromium e2e resets the explicit test database and covers booking acquisition plus first-use Manager intake → observed qualified-prospect goal drift → explicit reconciliation → globally ranked focus explanation → visible plan/provider-context disclosure → opt-in cadence → context gaps → profile/lineup completion → visible healthy profile-source knowledge → grounded context explanation → natural plan explanation → explicit memory proposal → reviewed save → persisted memory after reload → helpful answer promotion → progress/eval → conversational two-option proposal → accepted open draft → band framing → recorded choice → due review → conversational lesson → task block/reason/wait/date → ranked follow-through → grounded blocker chat → actionable gig readiness → phone day-of task completion → Manager-generated project milestones → assets/budget → Manager verification → offer/agreement PDF → invoice/deposit → expense/settlement → completed-show facts → grounded outcome review/chat. The offline `manager_os_v13` / `manager_evals_v14` gate has 24 golden checks plus local owner-reviewed recommendation and response examples, including explicit safe-memory confirmation, sensitive-memory refusal, goal-record reconciliation, knowledge source precedence, competing-pressure ranking, memory-sensitivity projection, exact event/project action selection, missing-context, blocked-follow-through, conversation-decision, decision/outcome grounding, naturalness/meta-language/action-claim, and feedback-guidance rules. |
...
2. **Learning validation:** Review real band context and knowledge-refresh questions, explicit conversational memory proposals, goal-measurement source choices and drift, competing-priority ordering, commitments, decisions, and accepted show/project setup actions with working bands; compare expected results with observed show/project/business facts. Add or adjust measurement kinds, `manager_priority_v1` weights, or `manager_knowledge_v1` review windows only from reviewed operator evidence, never from a synthetic score alone; do not infer causality from one result or auto-activate a version. `manager_os_v13` is the current code-registered version.
