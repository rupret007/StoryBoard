...
| Events, availability, show advance, songs/setlists, projects | Done; structured operations workspace, actionable gig editor, phone day-of view with editable custom run-of-show checkpoints, practical ordered setlist/song editing, shared `setlist_summary_v1` timing truth, evidence-backed show-readiness scoring, event-bound Calendar/Drive state with explicit approval preparation, and dry-run-default Vault/Show Night catalog import (`pnpm catalog:import` / `POST /songs/import`) onto the current artist only. The dashboard's first operational row now shows the nearest active recorded gig, its exact assigned set and provenance, and booking pipeline state; empty and unavailable records remain visibly different, and draft/hold status is never promoted to confirmed. The sticky mobile Command surface is collapsed to one launcher by default so it does not cover this read; desktop behavior and the approval-first command form are unchanged. Default seed dry-runs the checked-in `app_api.json` shape and still leaves the song table empty unless `VAULT_CATALOG_APPLY=true`; `SEED_DEMO_OPS` is optional generic practice data, not a live catalog. |
...
| Offers, reviewed templates, PDF snapshots, invoices/manual payments, settlements | Internal workflow done; Travis still books the gig. Invoice PATCH and payments fail-closed (payment-derived status, voided invoices immutable). One settlement per event; concurrent finalize is serialized and freezes event expenses. Manager chat does not pay invoices, void them, settle shows, or apply booking terms. |
...
| Tests | 2026-08-27 dashboard show/set/booking lift: Prisma generation; root typecheck/lint; 31/31 shared tests; 274/274 API tests; both production builds; 99/99 `manager_evals_v44` checks at 100% safety; desktop/mobile visual review and a loopback interaction smoke covering the compact Command launcher, expansion/collapse, desktop form, horizontal overflow, and browser runtime errors. Last full hosted package (2026-07-13) also recorded 5/5 Postgres workflows across all 40 migrations; catalog import added song/setlist `sourceKey` (41 migrations after apply). 15/15 Chromium journeys; no Prisma schema drift; no relationship-integrity violations (plus one expected non-fatal skip on older DB snapshots without `ApprovalReconciliation`); and rebuilt Compose health, readiness, Dev-login session, and authenticated-Dashboard smoke. The suites emit one tracked, non-fatal `pg@8.14.1` concurrent-query deprecation warning. |
