...
never reads as "Sept 14" on phones west of UTC. The Dashboard **Overdue by due
date** panel and the Tasks overdue grouping still compared the stored midnight
instant to the device clock and rendered with `toLocaleDateString()`, so the
same calendar-day shift and a "due later today" false overdue could appear on
Command center. Active project cards and project milestone lines had the same
display shift. Those surfaces now reuse `describeTaskDueDate` (`task_due_date_v1`)
for UTC calendar labels plus `(overdue)` / `(today)` flags, and they classify
overdue by `timing === "past"` only. Presentational reuse of the existing helper
— no API, schema, provider, or send change. Closed #21 remains closed; Travis
still books. Marker: `MAX_CLAUDE_STORYBOARD_20260913_1802`.
