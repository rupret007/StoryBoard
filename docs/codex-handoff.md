...
| Events, availability, show advance, songs/setlists, projects | Done; structured operations workspace, actionable gig editor, phone day-of view with editable custom run-of-show checkpoints, practical ordered setlist/song editing, shared `setlist_summary_v1` timing truth, and evidence-backed show-readiness scoring shipped |
...
| Tests | Compiled `node:test` (143 API cases plus 5 shared cases) plus opt-in Postgres integration coverage for tenant links, task prerequisite idempotency/cycles/date order/completion guards, custom event schedule ownership/lifecycle, setlist item replacement/summary/foreign-song rejection, tenant-isolated operating evidence and persisted calibration traces, dependency-aware work sequencing, tenant-isolated goal paths, stale goal-path acceptance preflight, linked-task traces, minimum/cap/exact goal semantics and PATCH preservation, structured conversation continuity, explicit natural-feedback targeting, reviewed context answers, shared-task creation, source-bound existing-task updates, and direct source-bound task assignment, plus source-bound atomic project-plus-plan creation, timezone-safe event/initial-lineup creation, and optimistic one-member event-availability updates, bounded conversation summaries and cross-tenant history rejection, exact named-record subject resolution, per-operator answer-review selection/refill, owner-only response-eval and finished-recommendation triage/promotion, stable-key review cutoff, and provider bypass, linked task owners, capacity-check-in history/premise changes, and Manager assignment acceptance, Manager intake/plans, goal reconciliation, explicit memory, code-owned coaching/provider bypass, cadence, context and knowledge integrity, priorities and commitments, decisions and outcomes, feedback/evals, event/project setup, booking campaigns, payments, settlements, roles, Telegram binding, and audits. Four Chromium journeys reset the explicit test database and cover booking acquisition; practical song/setlist creation, ordering, transitions, incomplete-to-timed duration; and first-use Manager intake → context and operating-evidence review → one-question context answer and reviewed profile save → new conversation → round-trip thread switching → capacity check-ins → team-load review → role-grounded task assignment and grounded “why?” follow-up → goal reconciliation and cap finality → personalized learning prompt → plain-language coaching → grounded conversation → immediate-turn natural answer feedback → answer review/refill → owner response-eval promotion → owner recommendation-outcome promotion and metric refresh → memory/decisions → task prerequisite creation → goal-to-next-work review → ready/waiting sequence review → commitment follow-through → gig/run-of-show/project operations → exact invoice balance selection → deal/invoice/settlement → outcome review → reviewed shared-task creation, duplicate refusal, reviewed assignment, reviewed completion, reviewed release-project creation with dated milestones, reviewed event creation with unknown lineup availability, and reviewed member availability before readiness. The offline `manager_os_v31` / `manager_evals_v34` gate has 67 golden checks plus local owner-reviewed recommendation and response examples, including explicit untimed-setlist honesty, reviewed context staging/sensitive-detail refusal, reviewed shared-task creation, update, and assignment staging, reviewed timezone-safe event staging and source-bound one-member availability changes, secret/implicit-work/prerequisite-bypass refusal, unavailable-owner refusal, explicit natural verdict targeting and action/completion refusal, structured follow-up explanation/currentness/clarification, pronoun-action refusal, exact show/invoice selection, ambiguous-name clarification, lumpy-goal forecast refusal, provisional budget caps, exact-target deadline misses, goal-path reuse/orphan prevention, prerequisite-aware ordering, stale booking calibration, missing-money honesty, direct evidence explanation, saved custom checkpoint grounding, current capacity, unavailable-member exclusion, unique versus ambiguous team assignment, novice settlement coaching, deal-structure comparison, unknown-topic clarification, safe-memory confirmation/refusal, goal and knowledge integrity, pressure ranking, exact internal action selection, decision/outcome grounding, naturalness, false-action rejection, and provider-context sensitivity. |
...
5. **Hosted CI maintenance:** GitHub run `29219555257` on pushed commit
   `1336997` passed typecheck, lint, unit, database, build, and container smoke,
   but the intentionally long Manager Chromium journey hit Playwright's
   default 30-second per-test timeout at 32 seconds. The same journey passes
   locally, and the expanded four-journey suite passes locally in 27 seconds.
   A test-specific 90-second hosted timeout is the focused fix; it remains
   unapplied because the CI-repair workflow requires explicit user approval.
