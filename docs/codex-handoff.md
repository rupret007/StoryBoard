...
2. **Learning validation:** Review real band context questions and decisions with working bands; compare expected results with observed show/project/business facts. Adjust weights only from reviewed operator evidence, never from a synthetic score alone; do not infer causality from one result or auto-activate a version. `manager_os_v7` is the current code-registered version.
