...
2. **Learning validation:** Review real band context, novice coaching questions, knowledge-refresh questions, explicit conversational memory proposals, goal-measurement source choices and drift, competing-priority ordering, commitments, decisions, and accepted show/project setup actions with working bands; compare expected results with observed show/project/business facts. Add or adjust coaching concepts, measurement kinds, `manager_priority_v1` weights, or `manager_knowledge_v1` review windows only from reviewed operator evidence, never from a synthetic score alone; do not infer causality from one result or auto-activate a version. `manager_os_v14` is the current code-registered version.
