...
const setlistFields = { name: z.string().trim().min(1).max(240), status: z.enum(["draft","active","archived"]), notes: z.string().trim().max(2000).nullable().optional(), sourceKey: z.string().trim().min(1).max(200).nullable().optional(), items: z.array(setlistItemSchema).max(100) } as const;
