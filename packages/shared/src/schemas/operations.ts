import { z } from "zod";
import { nullableSafeHttpUrl, safeHttpUrl } from "../operator-href";
...
const setlistItemSchema = z.object({ songId: z.string().trim().min(1).nullable().optional(), itemType: z.enum(["song","break","note"]).default("song"), label: z.string().trim().max(240).nullable().optional(), transitionNotes: z.string().trim().max(1000).nullable().optional() }).strict().superRefine((value, context) => {
  ...
  if (value.itemType !== "song" && !value.label) context.addIssue({ code: "custom", path: ["label"], message: "Breaks and notes need a label" });
});
...
export const projectPatchSchema = projectCreateSchema.partial().extend({
  ...
  assets: z.array(z.object({ label: z.string().max(200), url: safeHttpUrl }).strict()).max(50).optional()
}).strict();
...
export const paymentRecordSchema = z.object({ idempotencyKey: z.string().trim().min(1).max(200), amountMinor: z.number().int().positive(), currency: z.string().trim().length(3).default("USD"), method: z.string().trim().min(1).max(80), reference: z.string().trim().max(200).nullable().optional(), evidenceUrl: nullableSafeHttpUrl, receivedAt: z.string().datetime({ offset: true }) }).strict();
export const expenseCreateSchema = z.object({ eventId: nullableId, projectId: nullableId, category: z.string().trim().min(1).max(100), description: z.string().trim().min(1).max(500), amountMinor: z.number().int().positive(), currency: z.string().trim().length(3).default("USD"), incurredAt: z.string().datetime({ offset: true }), receiptUrl: nullableSafeHttpUrl }).strict().superRefine((value, ctx) => { if (!value.eventId && !value.projectId) ctx.addIssue({ code: "custom", path: ["eventId"], message: "An event or project is required." }); });
export const expensePatchSchema = z.object({ eventId: nullableId, projectId: nullableId, category: z.string().trim().min(1).max(100).optional(), description: z.string().trim().min(1).max(500).optional(), amountMinor: z.number().int().positive().optional(), currency: z.string().trim().length(3).optional(), incurredAt: z.string().datetime({ offset: true }).optional(), receiptUrl: nullableSafeHttpUrl }).strict();
export const settlementCreateSchema = z.object({ eventId: z.string().trim().min(1), currency: z.string().trim().length(3).default("USD"), grossMinor: z.number().int().nonnegative().default(0), notes: z.string().trim().max(3000).nullable().optional(), splits: z.array(z.object({ bandMemberId: z.string().trim().min(1), basisPoints: z.number().int().min(0).max(10000) }).strict()).max(50).default([]) }).strict().superRefine((value, ctx) => { if (value.splits.length && value.splits.reduce((sum, split) => sum + split.basisPoints, 0) !== 10000) ctx.addIssue({ code: "custom", path: ["splits"], message: "Member split basis points must total 10000." }); });
export const settlementPatchSchema = z.object({ grossMinor: z.number().int().nonnegative().optional(), notes: z.string().trim().max(3000).nullable().optional(), splits: z.array(z.object({ bandMemberId: z.string().trim().min(1), basisPoints: z.number().int().min(0).max(10000) }).strict()).max(50).optional() }).strict().superRefine((value, ctx) => { if (value.splits?.length && value.splits.reduce((sum, split) => sum + split.basisPoints, 0) !== 10000) ctx.addIssue({ code: "custom", path: ["splits"], message: "Member split basis points must total 10000." }); });
