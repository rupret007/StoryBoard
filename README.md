...
Prompt/policy version `manager_os_v33` and its `manager_evals_v39` offline eval
...
| `pnpm catalog:import` | **Default way to populate songs.** No `--source` dry-runs the checked-in `app_api.json` shape. A local Vault export imports **Rad Dad only**; writer/hybrid/parked rows stay out unless opted in. `--apply` writes. Never a remote URL. |
...
tests, both production builds, and 93/93 `manager_evals_v39` checks at 100%
